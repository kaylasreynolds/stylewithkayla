...
Kayla can preview the counts and run maintenance from `/admin/privacy`. The same idempotent operation is available at `POST /api/internal/maintenance` for a future external scheduler. Scheduled calls require a secret `MAINTENANCE_SECRET` bearer token stored in the hosting environment. No scheduler or secret is enabled in the private prototype yet; enabling one requires selecting an approved scheduler. Run the admin operation after each release and at least monthly until that choice is made.
