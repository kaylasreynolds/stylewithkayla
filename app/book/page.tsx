...
export default function Home() {
  ...
  const summaryService = selectedService ?? visibleServices[0] ?? services[0];
  ...
  useEffect(() => {
    fetch("/api/services", { cache: "no-store" })
      .then(async (response) => {
        ...
        setServices(
          servicePresentation.map((presentation) => {
            const stored = payload.data.services.find(
              (item) => item.code === presentation.id,
            );
            return stored
              ? {
                  ...presentation,
      ...
      .catch(() =>
        setBookingError("Services could not be refreshed. Please try again."),
      );
  }, []);
  ...
  useEffect(() => {
    ...
    setAudience(validService.audience);
    setServiceId(validService.id);
  }, [services]);
  useEffect(() => {
    if (!serviceId) {
      setSlotsByDate({});
    ...
    const from = dateKeyInBoise(today),
      to = dateKeyInBoise(maxDate);
    Promise.resolve()
      .then(() => {
        setLoadingTimes(true);
        setBookingError("");
        return fetch(
          `/api/availability?serviceCode=${encodeURIComponent(serviceId)}&from=${from}&to=${to}`,
      ...
      .then((data) => {
        const grouped = data.slots.reduce<Record<string, Slot[]>>(
          (result, slot) => {
        ...
        setSelectedDate((current) => (grouped[current]?.length ? current : ""));
        setSelectedTime((current) =>
          data.slots.some((slot) => slot.startsAt === current) ? current : "",
        );
      })
      .catch((error) => {
  ...
  function setAudienceAndClear(item: "Women" | "Men") {
  ...
  function validateDetails() {
    ...
    const emailInput = fieldRefs.current.email as HTMLInputElement | null;
    ...
    if (!form.returning) errors.returning = "Choose yes or no.";
    if (selectedService?.isEvent) {
      if (!form.eventType) errors.eventType = "Choose the type of event.";
  ...
  function fieldErrorProps(name: string) {
    return {
  ...
  async function submitRequest() {
    ...
    try {
      ...
      if (!response.ok) {
        if (mapApiErrors(payload.error)) return;
        throw new Error(
          payload.error?.message || "Your request could not be submitted.",
        );
      }
  ...
  function canContinue() {
    ...
    if (step === 3) {
      const conditional = selectedService?.isEvent
        ? Boolean(form.eventType && form.eventDate)
        : true;
      return Boolean(
        form.name &&
        form.email &&
      ...
      <div className="site-shell">
        ...
        <main className="success-wrap">
          ...
          <p className="success-lead">
            Your appointment is pending until Kayla reviews and confirms it.
          </p>
          ...
          <p className="request-reference">
            Request reference: <strong>{publicReference}</strong>
          </p>
          <div className="next-steps">
            <div>
              <span>1</span>
              <p>
                <strong>Kayla reviews your request</strong>
                <br />
            ...
            <div>
              <span>2</span>
              <p>
                <strong>You receive confirmation</strong>
                <br />
            ...
            <div>
              <span>3</span>
              <p>
                <strong>Complete your Style Profile</strong>
                <br />
      ...
      <main className="booking-page">
        <aside className="booking-intro">
          ...
          <p className="intro-copy">
            {step === 1 &&
              "Select the service that best matches what you need. Every appointment is complimentary and personalized."}
            {step === 2 &&
              "Choose an available date and time. Your appointment will be held as pending until I review your request."}
            {step === 3 &&
              "A few details help me confirm your request and send the correct Style Profile after approval."}
            {step === 4 &&
              "Make sure everything looks right before submitting. Your selected time will be held while I review it."}
          </p>
                ...
                >
                  {(["Women", "Men"] as const).map((item) => (
                    <button
                      ...
                      onClick={() => setAudienceAndClear(item)}
                    >
                      {item}
                    </button>
                  ))}
              ...
              <div className="calendar-layout">
                <div className="calendar-panel">
                  ...
                  <p className="availability-note">
                    <span /> Available Tuesday–Saturday, 24+ hours ahead
                  </p>
                </div>
                    ...
                    {loadingTimes ? (
                      ...
                      slots.map((slot) => (
                        <button
                          key={slot.startsAt}
                          className={
                            selectedTime === slot.startsAt ? "selected" : ""
                          }
                          onClick={() => setSelectedTime(slot.startsAt)}
                        >
                          {selectedTime === slot.startsAt && <span>✓</span>}
                          {timeInBoise(slot.startsAt)}
                        </button>
                      ))
                    ) : (
                      <p className="loading-times">
                        No routine times are available for this date.
                      </p>
                    )}
                  </div>
                  <p className="time-contact">
                    Don&apos;t see a time that works?{" "}
                    <a href="mailto:kayla.reynolds@macys.com?subject=Appointment%20Time%20Request">
                      Send me a message.
                    </a>
                  </p>
                </div>
            ...
            >
              ...
              <div className="form-grid">
                ...
                <div
                  ...
                  style={{
                    gridTemplateColumns:
                      "minmax(250px, 300px) minmax(360px, 520px)",
                ...
                >
                      ...
                      {["Yes", "No"].map((value) => (
                        <button
                          ...
                          ref={
                            value === "Yes"
                              ? (node) => {
                                  fieldRefs.current.returning = node;
                                }
                              : undefined
                          }
                  ...
                  <label className="detail-group">
                    <span style={{ display: "block", lineHeight: 1.35 }}>
                      ...
                      >
                        Sizes and style preferences will be requested after your
                ...
                {selectedService?.isEvent && (
                  <>
                    <label>
                      ...
                      <select
                        ref={(node) => {
                          fieldRefs.current.eventType = node;
                        }}
                      ...
                      >
                        ...
                        <option>Gala or Formal Event</option>
                    ...
                    <label>
                      ...
                      <input
                        ref={(node) => {
                          fieldRefs.current.eventDate = node;
                        }}
                        type="date"
                ...
                <label className="privacy-check full">
                  <input
                    ...
                    {...fieldErrorProps("privacy")}
                  />
                  <span>
                    I understand my information will be used to manage and
            ...
            >
              ...
              <div className="pending-callout">
                ...
                <p>
                  ...
                  <br />
                  Your selected time will be held while Kayla reviews it. You
                  will receive a confirmation or an alternate-time option.
                </p>
              </div>
            </form>
          )}
...
function Announcement() {
  return (
    <div className="announcement-bar">
      Complimentary Personal Styling Appointments | 20% Off For First Time
      Clients*
    </div>
  );
}
  ...
  return (
    <header className="site-header" aria-label="Site header">
      <div className="container header-inner">
        ...
        <nav className="site-nav" aria-label="Booking navigation">
    ...
    <footer className="site-footer" id="contact">
      <div className="container footer-inner">
        ...
        >
          ...
          <span className="footer-link--inactive" aria-disabled="true">
            Guides
          </span>
          <span className="footer-link--inactive" aria-disabled="true">
            FAQ
      ...
      <div className="container privacy-notice" id="privacy">
        ...
        <p>
          Your information is used only to review, manage, and prepare for your
          styling appointment. A pending request holds the selected time until
          Kayla confirms, declines, releases, or replaces it. Confirmed clients
          receive a private Style Profile link that expires 30 days after issue.
          Written booking and profile history is retained for two years after
          the most recent completed appointment. Contact Kayla to request
  ...
  return (
    <div className={`appointment-summary ${compact ? "compact" : ""}`}>
      ...
      <div className="summary-content">
        ...
        <p>{service.duration} minutes · Complimentary</p>
