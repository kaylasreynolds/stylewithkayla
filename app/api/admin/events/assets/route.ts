...
import { EVENT_IMAGE_MAX_BYTES, inspectEventImage } from "@/lib/server/event-images";
...
export async function POST(request: Request) {
  return withApi(async requestId => {
    ...
    if (declaredLength > EVENT_IMAGE_MAX_BYTES + 64 * 1024) throw new ApiError(413, "EVENT_IMAGE_TOO_LARGE", "Event images must be 5 MB or smaller.");
    const form = await request.formData(), file = form.get("file");
    if (!(file instanceof File)) throw new ApiError(422, "FILE_REQUIRED", "Choose a JPG, PNG, or WebP image.");
    if (file.size > EVENT_IMAGE_MAX_BYTES) throw new ApiError(413, "EVENT_IMAGE_TOO_LARGE", "Event images must be 5 MB or smaller.");
