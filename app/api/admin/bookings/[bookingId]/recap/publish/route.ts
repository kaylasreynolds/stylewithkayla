...
export async function POST(request: Request, ctx: Params) { return withApi(async id => {
  ...
  return dataResponse(await publishRecapSummary({ db, bookingId, recapId: String(source.recap.recapId), recipient: typeof source.client.email === "string" ? source.client.email : null, content, origin: new URL(request.url).origin }), 200, id);
}); }
