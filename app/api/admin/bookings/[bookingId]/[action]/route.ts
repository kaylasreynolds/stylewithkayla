...
type B={id:string;status:string;serviceCode:string;profileType:string|null;clientId:string;clientEmail:string;holdId:string|null;holdStartsAt:number|null;holdEndsAt:number|null};
...
export async function POST(request:Request,ctx:Params){return withApi(async id=>{
  ...
  if(!new Set(["confirm","decline","release-hold","propose-time","cancel","complete"]).has(action))throw new ApiError(404,"ACTION_NOT_FOUND","Booking action not found.");
  const body=await readJsonObject(request);rejectUnexpectedKeys(body,["expectedStatus","reason","requestedStartAt","overrideConflict"]);
  ...
  let profileAccessUrl:string|null=null,profileAccessExpiresAt:number|null=null;
  ...
  if(action==="confirm"){
    ...
    await db.batch([
      ...
      history(db,bookingId,expected,"confirmed",admin.subjectId,reason,{action,idempotencyKeyHash:keyHash,profileLinkExpiresAt:new Date(profileAccessExpiresAt).toISOString()},now),communication(db,bookingId,b.clientEmail,"booking_confirmed",now)
    ]);
  }else if(action==="decline"){
    const why=requiredString(body.reason,"reason",500);await db.batch([db.prepare(`UPDATE bookings SET status='declined',declined_at=?,updated_at=? WHERE id=? AND status=?`).bind(now,now,bookingId,expected),db.prepare(`UPDATE booking_holds SET active=0,released_at=?,release_reason=? WHERE booking_id=? AND active=1 AND EXISTS(SELECT 1 FROM bookings WHERE id=? AND status='declined')`).bind(now,why,bookingId,bookingId),history(db,bookingId,expected,"declined",admin.subjectId,why,{action,idempotencyKeyHash:keyHash},now),communication(db,bookingId,b.clientEmail,"booking_declined",now)]);
  }else if(action==="release-hold"){
    if(!b.holdId)throw new ApiError(409,"NO_ACTIVE_HOLD","This request no longer has an active hold.");const why=reason??"Released by Kayla";await db.batch([db.prepare(`UPDATE booking_holds SET active=0,released_at=?,release_reason=? WHERE id=? AND active=1`).bind(now,why,b.holdId),history(db,bookingId,expected,expected,admin.subjectId,why,{action,idempotencyKeyHash:keyHash},now)]);
  }else if(action==="propose-time"){
    const text=requiredString(body.requestedStartAt,"requestedStartAt",40),start=Date.parse(text);if(!Number.isFinite(start)||new Date(start).toISOString()!==text)throw validation("requestedStartAt","Choose a valid available time.");const date=localDateKey(start,"America/Boise"),availability=await calculateAvailability(db,b.serviceCode,date,date,Date.now(),bookingId),slot=availability.slots.find(x=>x.startsAt===text);if(!slot)throw new ApiError(409,"SLOT_UNAVAILABLE","That alternate time is unavailable.");const end=Date.parse(slot.endsAt),holdId=crypto.randomUUID();await db.batch([db.prepare(`UPDATE booking_holds SET active=0,released_at=?,release_reason='Replaced by proposed time' WHERE booking_id=? AND active=1`).bind(now,bookingId),db.prepare(`INSERT INTO booking_holds(id,booking_id,kind,starts_at,ends_at,active,created_at) SELECT ?,id,'proposed',?,?,1,? FROM bookings WHERE id=? AND status=? AND NOT EXISTS(SELECT 1 FROM booking_holds h WHERE h.active=1 AND h.booking_id<>? AND h.starts_at<? AND h.ends_at>?)`).bind(holdId,start,end,now,bookingId,expected,bookingId,end,start),db.prepare(`UPDATE bookings SET status='change_proposed',proposed_start_at=?,proposed_end_at=?,updated_at=? WHERE id=? AND status=? AND EXISTS(SELECT 1 FROM booking_holds WHERE id=? AND active=1)`).bind(start,end,now,bookingId,expected,holdId),history(db,bookingId,expected,"change_proposed",admin.subjectId,reason,{action,idempotencyKeyHash:keyHash},now),communication(db,bookingId,b.clientEmail,"alternate_time_proposed",now)]);
  }else if(action==="cancel"){
    const why=requiredString(body.reason,"reason",500);await db.batch([db.prepare(`UPDATE bookings SET status='cancelled',cancelled_at=?,updated_at=? WHERE id=? AND status='confirmed'`).bind(now,now,bookingId),db.prepare(`UPDATE booking_holds SET active=0,released_at=?,release_reason=? WHERE booking_id=? AND active=1`).bind(now,why,bookingId),db.prepare(`UPDATE private_access_tokens SET revoked_at=? WHERE booking_id=? AND revoked_at IS NULL`).bind(now,bookingId),history(db,bookingId,"confirmed","cancelled",admin.subjectId,why,{action,idempotencyKeyHash:keyHash},now),communication(db,bookingId,b.clientEmail,"booking_cancelled",now)]);
  }else{
    const retention=addCalendarYears(now,2);await db.batch([db.prepare(`UPDATE bookings SET status='completed',completed_at=?,updated_at=? WHERE id=? AND status='confirmed'`).bind(now,now,bookingId),db.prepare(`UPDATE booking_holds SET active=0,released_at=?,release_reason='Appointment completed' WHERE booking_id=? AND active=1`).bind(now,bookingId),db.prepare(`UPDATE clients SET last_completed_appointment_at=?,retention_delete_after=?,updated_at=? WHERE id=?`).bind(now,retention,now,b.clientId),db.prepare(`UPDATE style_profiles SET retention_delete_after=?,updated_at=? WHERE booking_id=?`).bind(retention,now,bookingId),history(db,bookingId,"confirmed","completed",admin.subjectId,reason,{action,idempotencyKeyHash:keyHash,retentionDeleteAfter:new Date(retention).toISOString()},now)]);
  }
  const updated=await load(db,bookingId);if(!updated||(action!=="release-hold"&&updated.status===expected))throw new ApiError(409,"BOOKING_STATE_CHANGED","This request changed. Refresh before continuing.");return dataResponse({bookingId,status:updated.status,holdActive:Boolean(updated.holdId),profileAccessUrl,profileAccessExpiresAt:profileAccessExpiresAt?new Date(profileAccessExpiresAt).toISOString():null},200,id);
});}
...
function communication(db:D1Database,id:string,email:string,key:string,now:number){return db.prepare(`INSERT INTO communications(id,booking_id,channel,template_key,recipient,status,metadata,created_at) SELECT ?,id,'email',?,?,'recorded',?,? FROM bookings WHERE id=?`).bind(crypto.randomUUID(),key,email,JSON.stringify({deliveryDeferred:true}),now,id);}
