...
type B = { id: string; status: string; serviceCode: string; durationMinutes: number; profileType: string | null; clientId: string; clientEmail: string; holdId: string | null; holdStartsAt: number | null; holdEndsAt: number | null };

export async function POST(request: Request, ctx: Params) {
  return withApi(async id => {
    const adminEmail = requireAdmin(request), { bookingId, action } = await ctx.params;
    ...
    rejectUnexpectedKeys(body, ["expectedStatus", "reason", "requestedStartAt", "overrideConflict", "selectionSource", "overrideConfirmed"]);
    ...
    if (action === "confirm") {
      ...
      if (!Number.isFinite(start) || new Date(start).toISOString() !== text) throw validation("requestedStartAt", "Choose a valid Boise date and time.");
      if (start <= now) throw validation("requestedStartAt", "Choose a future date and time.");
      if (!isBoiseQuarterHour(start)) throw validation("requestedStartAt", "Choose a time in 15-minute increments.");
      const end = start + b.durationMinutes * 60_000;
      const conflict = await db.prepare(`SELECT id FROM booking_holds WHERE active=1 AND booking_id<>? AND starts_at<? AND ends_at>? UNION ALL SELECT id FROM bookings WHERE id<>? AND status='confirmed' AND confirmed_start_at<? AND confirmed_end_at>? LIMIT 1`).bind(bookingId, end, start, bookingId, end, start).first();
      if (conflict) throw new ApiError(409, "APPOINTMENT_CONFLICT", "That time conflicts with another appointment or active hold.", { requestedStartAt: "Choose a time that does not overlap another appointment." });
      const date = localDateKey(start, "America/Boise"), availability = await calculateAvailability(db, b.serviceCode, date, date, now, bookingId), outsidePublicAvailability = !availability.slots.some(slot => slot.startsAt === text);
      const overrideConfirmed = body.overrideConfirmed === true;
      if (outsidePublicAvailability && !overrideConfirmed) throw validation("overrideConfirmed", "Confirm that you reviewed this custom time outside public availability.");
      const requestedSource = body.selectionSource === "public_availability" ? "public_availability" : "custom";
