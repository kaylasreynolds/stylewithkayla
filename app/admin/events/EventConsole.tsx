...
import { buildEventPayload, defaultEventCostLabel, EventSubmissionGuard, eventImageFileError, hasEventOffer, isUploadedEventImage, readUploadResponse, withoutEventOffer } from "@/lib/event-editor-client";
...
async function submit(publish = false) {
 ...
 return <Shell title={eventId?'Edit event':'Create event'} actions={<Link href="/admin/events" className="event-button event-button--secondary">Return to Events</Link>}>
  ...
  <fieldset className="event-image-field"><legend>2. Event Image</legend><p>JPG, PNG, or WebP. Images are resized without cropping and must be no more than 5 MiB after optimization.</p><p className="event-image-notice">The uploaded image is displayed in full. It is never cropped or upscaled.</p>{(localPreview||asset)&&<img src={localPreview||asset?.previewUrl} alt={imageAlt||'Selected event image preview'}/>}<div><label className="event-button event-button--secondary">{asset?'Replace image':'Upload image'}<input type="file" accept=".jpg,.jpeg,.png,.webp" onChange={e=>e.target.files?.[0]&&upload(e.target.files[0])}/></label>{(localPreview||asset)&&<button type="button" className="event-link-button" onClick={()=>{uploadSequence.current++;if(localPreview)URL.revokeObjectURL(localPreview);setLocalPreview('');setImageSizes(null);setImageStatus('idle');setImageError('');setAsset(null);set('imageAssetId',null);set('imageAlt','')}}>Remove image</button>}</div><span aria-live="polite">{imageStatus==='optimizing'?'Optimizing image…':imageStatus==='uploading'?'Uploading image…':imageStatus==='uploaded'?'Image uploaded':imageStatus==='failed'?'Upload failed':''}</span>{imageError&&<p className="event-alert" role="alert">{imageError}</p>}{imageSizes&&<p>Original: <strong>{formatImageBytes(imageSizes.original)}</strong> · Optimized: <strong>{formatImageBytes(imageSizes.optimized)}</strong></p>}{progress!==null&&<><progress value={progress} max="100"/><span aria-live="polite">Uploading: {progress}%</span></>}{(localPreview||asset)&&<label>Meaningful alternative text<textarea value={imageAlt} onChange={e=>set('imageAlt',e.target.value)} maxLength={240} aria-invalid={!!errors.imageAlt}/><small>Briefly describe the image for someone who cannot see it. Do not repeat text elsewhere on the page unless it is essential to understanding the image.</small><small>{imageAlt.length} / 240 characters</small>{errors.imageAlt&&<small className="event-field-error">{errors.imageAlt}</small>}</label>}</fieldset>
<fieldset>
