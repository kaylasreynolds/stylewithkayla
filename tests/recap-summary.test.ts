...
import { encryptPrivateToken } from "../lib/server/crypto";
import { deliverStyleSummaryEmail, loadCanonicalStyleSummary } from "../lib/server/style-summary-delivery";
import { renderStyleSummaryEmail } from "../lib/server/style-summary-email";
...
const canonicalMigration = readFileSync(new URL("../drizzle/20260831120000_recap_summary_canonical_token.sql", import.meta.url), "utf8");
...
const previewRoute = readFileSync(new URL("../app/api/admin/bookings/[bookingId]/recap/preview/route.ts", import.meta.url), "utf8");
const previewComponent = readFileSync(new URL("../app/admin/recap-preview/[bookingId]/RecapPreview.tsx", import.meta.url), "utf8");
...
function fixture() {
  const db = new DatabaseSync(":memory:"); db.exec("PRAGMA foreign_keys=ON"); apply(db, baseline); apply(db, migration); apply(db,canonicalMigration);
...
async function publish(db: DatabaseSync, now: number, serviceName: string, raw = "r".repeat(43)) {
  return publishRecapSummary({ db: d1(db), bookingId: "booking-summary", recapId: "recap-summary", content: liveContent(db, serviceName), origin: "https://style.test",encryptionKey:"unused" }, { now: () => now, id: () => `generated-${++idSequence}`, rawToken: () => raw, hashToken: async value => hash(value),encryptToken:async()=>({tokenCiphertext:"cipher",tokenIv:"iv",tokenAuthTag:"tag"}) });
}
...
test("production summary access enforces purpose and revocation without expiring the canonical recap link", async () => {
  ...
  assert.equal((await findRecapSummaryAccess(d1(db),raw,now+50*365*24*60*60*1000,async value=>hash(value))).bookingId,"booking-summary");
...
test("publication stores one encrypted canonical token and does not mark or log delivery",async()=>{const{db,now,service}=fixture();await publish(db,now,service.name);const token=db.prepare("SELECT token_hash AS tokenHash,expires_at AS expiresAt,token_ciphertext AS tokenCiphertext,token_iv AS tokenIv,token_auth_tag AS tokenAuthTag FROM private_access_tokens WHERE purpose='recap_summary'").get() as Record<string,unknown>;assert.equal(typeof token.tokenHash,"string");assert.equal(token.expiresAt,null);assert.deepEqual([token.tokenCiphertext,token.tokenIv,token.tokenAuthTag],["cipher","iv","tag"]);assert.deepEqual({...db.prepare("SELECT sent_at AS sentAt,recipient FROM recap_summaries").get()!},{sentAt:null,recipient:null});assert.equal((db.prepare("SELECT COUNT(*) count FROM communications WHERE template_key='style_summary_follow_up'").get() as {count:number}).count,0);});

test("canonical URL survives reload and send/resend without creating tokens or summaries",async()=>{const{db,now,service}=fixture(),raw="z".repeat(43),keyBytes=new Uint8Array(32).fill(7),key=Buffer.from(keyBytes).toString("base64url");await publishRecapSummary({db:d1(db),bookingId:"booking-summary",recapId:"recap-summary",content:liveContent(db,service.name),origin:"https://style.test",encryptionKey:key},{now:()=>now,id:()=>`real-${++idSequence}`,rawToken:()=>raw,hashToken:async value=>hash(value),encryptToken:value=>encryptPrivateToken(value,key)});const reloaded=await loadCanonicalStyleSummary(d1(db),"booking-summary","https://style.test",key);assert.equal(reloaded.styleSummaryUrl,`https://style.test/style-summary/${raw}`);const urls:string[]=[];const send=async(_config:never,message:{html:string})=>{const match=message.html.match(/href="([^"]+)"/);urls.push(match![1]);};for(let attempt=1;attempt<=2;attempt++)await deliverStyleSummaryEmail(d1(db),"booking-summary","https://style.test",{key,config:{tenantId:"t",clientId:"c",clientSecret:"s",mailbox:"m",replyTo:"r",notificationTo:"n"},send:send as never,now:()=>now+attempt,id:()=>`communication-${attempt}`});assert.deepEqual(urls,[reloaded.styleSummaryUrl,reloaded.styleSummaryUrl]);assert.equal((db.prepare("SELECT COUNT(*) count FROM private_access_tokens WHERE purpose='recap_summary'").get() as {count:number}).count,1);assert.equal((db.prepare("SELECT COUNT(*) count FROM recap_summaries").get() as {count:number}).count,1);assert.equal((db.prepare("SELECT COUNT(*) count FROM communications WHERE status='sent' AND template_key='style_summary_follow_up'").get() as {count:number}).count,2);assert.equal(JSON.stringify(db.prepare("SELECT metadata FROM communications").all()).includes(raw),false);});

test("legacy published summaries retain frozen previews and token access without exposing delivery actions",async()=>{const{db,now,service}=fixture(),raw="l".repeat(43),key=Buffer.from(new Uint8Array(32).fill(6)).toString("base64url"),frozen=liveContent(db,service.name);await publishRecapSummary({db:d1(db),bookingId:"booking-summary",recapId:"recap-summary",content:frozen,origin:"https://style.test",encryptionKey:key},{now:()=>now,rawToken:()=>raw,hashToken:async value=>hash(value),encryptToken:value=>encryptPrivateToken(value,key)});db.prepare("UPDATE private_access_tokens SET token_ciphertext=NULL,token_iv=NULL,token_auth_tag=NULL WHERE purpose='recap_summary'").run();db.prepare("UPDATE appointment_recaps SET what_we_solved='CHANGED' WHERE id='recap-summary'").run();const legacy=await loadCanonicalStyleSummary(d1(db),"booking-summary","https://style.test",key);assert.equal(legacy.styleSummaryUrl,null);assert.equal(legacy.linkRecoverable,false);assert.deepEqual(legacy.content,frozen);assert.deepEqual(await access(db,raw,now+1),frozen);await assert.rejects(deliverStyleSummaryEmail(d1(db),"booking-summary","https://style.test",{key,config:{tenantId:"t",clientId:"c",clientSecret:"s",mailbox:"m",replyTo:"r",notificationTo:"n"}}),error=>error instanceof ApiError&&error.status===409&&error.code==="STYLE_SUMMARY_LINK_UNAVAILABLE");assert.equal((db.prepare("SELECT COUNT(*) count FROM communications").get() as {count:number}).count,0);assert.match(previewRoute,/linkRecoverable:canonical\.linkRecoverable/);assert.match(previewComponent,/published&&!linkRecoverable/);assert.match(previewComponent,/This Style Summary was published before reusable links were enabled\. Its original private link cannot be recovered\./);assert.match(previewComponent,/linkRecoverable&&link&&/);});

test("send eligibility rejects unpublished, missing-email, and revoked summaries",async()=>{const key=Buffer.from(new Uint8Array(32).fill(8)).toString("base64url"),config={tenantId:"t",clientId:"c",clientSecret:"s",mailbox:"m",replyTo:"r",notificationTo:"n"};{const{db}=fixture();await assert.rejects(deliverStyleSummaryEmail(d1(db),"booking-summary","https://style.test",{key,config}),e=>e instanceof ApiError&&e.code==="STYLE_SUMMARY_NOT_PUBLISHED");}{const{db,now,service}=fixture();await publishRecapSummary({db:d1(db),bookingId:"booking-summary",recapId:"recap-summary",content:liveContent(db,service.name),origin:"https://style.test",encryptionKey:key},{now:()=>now,encryptToken:value=>encryptPrivateToken(value,key)});db.prepare("UPDATE clients SET email='' WHERE id='client-summary'").run();await assert.rejects(deliverStyleSummaryEmail(d1(db),"booking-summary","https://style.test",{key,config}),e=>e instanceof ApiError&&e.code==="CLIENT_EMAIL_UNAVAILABLE");db.prepare("UPDATE clients SET email='jamie@example.com'").run();db.prepare("UPDATE private_access_tokens SET revoked_at=? WHERE purpose='recap_summary'").run(now+1);await assert.rejects(deliverStyleSummaryEmail(d1(db),"booking-summary","https://style.test",{key,config}),e=>e instanceof ApiError&&e.code==="STYLE_SUMMARY_LINK_REVOKED");assert.equal((db.prepare("SELECT COUNT(*) count FROM communications").get() as {count:number}).count,0);}});

test("failed delivery is logged without changing publication, canonical token, or prior sent state",async()=>{const{db,now,service}=fixture(),key=Buffer.from(new Uint8Array(32).fill(9)).toString("base64url"),raw="f".repeat(43),config={tenantId:"t",clientId:"c",clientSecret:"s",mailbox:"m",replyTo:"r",notificationTo:"n"};await publishRecapSummary({db:d1(db),bookingId:"booking-summary",recapId:"recap-summary",content:liveContent(db,service.name),origin:"https://style.test",encryptionKey:key},{now:()=>now,rawToken:()=>raw,encryptToken:value=>encryptPrivateToken(value,key)});await deliverStyleSummaryEmail(d1(db),"booking-summary","https://style.test",{key,config,send:(async()=>{}) as never,now:()=>now+1,id:()=>"successful"});await assert.rejects(deliverStyleSummaryEmail(d1(db),"booking-summary","https://style.test",{key,config,send:(async()=>{throw new Error("Graph unavailable");}) as never,now:()=>now+2,id:()=>"failed"}),e=>e instanceof ApiError&&e.status===502);assert.deepEqual({...db.prepare("SELECT status,sent_at AS sentAt,recipient FROM recap_summaries JOIN appointment_recaps ON appointment_recaps.id=recap_summaries.recap_id").get()!},{status:"published",sentAt:now+1,recipient:"jamie@example.com"});assert.equal((db.prepare("SELECT status FROM communications WHERE id='failed'").get() as {status:string}).status,"failed");assert.equal((await loadCanonicalStyleSummary(d1(db),"booking-summary","https://style.test",key)).styleSummaryUrl,`https://style.test/style-summary/${raw}`);});

test("missing Microsoft configuration records a failed first send and leaves sent_at null",async()=>{const{db,now,service}=fixture(),key=Buffer.from(new Uint8Array(32).fill(10)).toString("base64url");await publishRecapSummary({db:d1(db),bookingId:"booking-summary",recapId:"recap-summary",content:liveContent(db,service.name),origin:"https://style.test",encryptionKey:key},{now:()=>now,encryptToken:value=>encryptPrivateToken(value,key)});await assert.rejects(deliverStyleSummaryEmail(d1(db),"booking-summary","https://style.test",{key,config:null,id:()=>"missing-config"}),e=>e instanceof ApiError&&e.status===502);assert.equal((db.prepare("SELECT status FROM communications WHERE id='missing-config'").get() as {status:string}).status,"failed");assert.equal((db.prepare("SELECT sent_at AS sentAt FROM recap_summaries").get() as {sentAt:number|null}).sentAt,null);assert.equal((db.prepare("SELECT status FROM appointment_recaps").get() as {status:string}).status,"published");});

test("follow-up email contains only the approved copy and canonical CTA",()=>{const url="https://style.test/style-summary/canonical",email=renderStyleSummaryEmail({firstName:"Jamie",styleSummaryUrl:url});assert.equal(email.subject,"Your Style Summary is ready");assert.match(email.html,/View Your Style Summary/);assert.match(email.html,new RegExp(url.replaceAll("/","\\/")));assert.match(email.text,new RegExp(url.replaceAll("/","\\/")));for(const excluded of ["What We Learned","Compliments You","Less Flattering","Outfit Formulas","Next Styling Moment","Kayla Note","PRIVATE"])assert.doesNotMatch(email.html,new RegExp(excluded));assert.doesNotMatch(email.html,/var\(--|display:grid|<link/i);});
