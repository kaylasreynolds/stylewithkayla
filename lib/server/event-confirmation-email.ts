...
const buildGoogleCalendarUrl = (input: ConfirmationEmailInput) => {
  const startsAt = input.appointmentStartsAt ?? input.eventStartsAt;
  const endsAt = input.appointmentEndsAt ?? input.eventEndsAt;
...
const buildEmailHtml = (input: ConfirmationEmailInput) => {
  ...
  const time = input.appointmentLabel || `${formatTime(scheduledStartsAt, input.timezone)}–${formatTime(scheduledEndsAt, input.timezone)}`;
