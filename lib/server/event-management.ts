...
import { eventSlugError } from "../event-slug";
...
export const eventLabels = ["Open House","Workshop","Community Event","Presell","Special Event","Custom"] as const;
export const legacyEventLabels = ["Appointment","RSVP","Drop-In","Styling Event","Brand Event","Limited Spots"] as const;
export const attendanceTypes = ["open_attendance","appointment_required","appointment_recommended","general_rsvp","information_only"] as const;
export const legacyAttendanceTypes = ["drop_in","invitation_only","interest_list"] as const;
export const costTypes = ["complimentary","paid","purchase_required","free_with_rsvp","not_applicable","custom"] as const;
...
function parseEventShape(value:Record<string,unknown>,partial=false,publishFormats=false){rejectUnexpectedKeys(value,EVENT_KEYS);const o:EventInput={};const take=(k:string,fn:(v:unknown)=>unknown,def?:unknown)=>{if(k in value)o[k]=fn(value[k]);else if(!partial&&def!==undefined)o[k]=def;};
  take("title",v=>str(v,"title",160),"");take("slug",v=>{const s=str(v,"slug",180),message=eventSlugError(s);if(message)throw validation("slug",message);return s;},"");take("eventLabel",v=>v===""?"":enumValue(v,"eventLabel",[...eventLabels,...legacyEventLabels]),"");take("customLabel",v=>str(v,"customLabel",80),"");take("shortDescription",v=>str(v,"shortDescription",320),"");take("description",v=>str(v,"description",5000),"");take("offer",v=>str(v,"offer",180),"");take("offerDetails",v=>str(v,"offerDetails",1000),"");take("offerTerms",v=>str(v,"offerTerms",1000),"");
  take("eventDate",v=>str(v,"eventDate",10),"");take("startTime",v=>str(v,"startTime",8),"");take("endTime",v=>str(v,"endTime",8),"");take("allDay",v=>bool(v,"allDay"),false);take("timezone",v=>requiredString(v,"timezone",64),"America/Boise");take("location",v=>str(v,"location",300),"");take("locationDetails",v=>str(v,"locationDetails",500),"");take("directionsUrl",v=>publishFormats?url(v,"directionsUrl"):str(v,"directionsUrl",2048),"");
  take("attendanceType",v=>v===""?"":enumValue(v,"attendanceType",[...attendanceTypes,...legacyAttendanceTypes]),"");take("capacity",v=>v===null||v===""?null:positiveInteger(v,"capacity",10000),null);take("unlimitedCapacity",v=>bool(v,"unlimitedCapacity"),false);take("maxGuests",v=>Number.isInteger(v)&&(v as number)>=0&&(v as number)<=20?v:(()=>{throw validation("maxGuests","Enter a whole number from 0 to 20.")})(),0);take("allowGuestNames",v=>bool(v,"allowGuestNames"),false);
  ...
  take("costType",v=>v===""?"":enumValue(v,"costType",costTypes),"");take("costLabel",v=>str(v,"costLabel",120),"");
  const defaultCostLabels: Record<string, string> = {complimentary:"Complimentary",purchase_required:"Purchase required",free_with_rsvp:"Free with RSVP",not_applicable:"Not applicable"};
  if(publishFormats&&!partial&&o.costType&&!o.costLabel&&defaultCostLabels[String(o.costType)])o.costLabel=defaultCostLabels[String(o.costType)];
  take("ctaLabel",v=>str(v,"ctaLabel",100),"");take("ctaAction",v=>v===""?"":enumValue(v,"ctaAction",ctaActions),"");take("ctaUrl",v=>publishFormats?url(v,"ctaUrl"):str(v,"ctaUrl",2048),"");take("ctaEmail",v=>publishFormats?email(v,"ctaEmail"):str(v,"ctaEmail",254),"");take("ctaPhone",v=>{const s=str(v,"ctaPhone",40);if(publishFormats&&s&&!/^\+?[0-9() .-]{7,40}$/.test(s))throw validation("ctaPhone","Enter a valid phone number.");return s;},"");take("sharingEnabled",v=>bool(v,"sharingEnabled"),true);take("shareMessage",v=>str(v,"shareMessage",1500),"");take("imageAssetId",v=>v===null||v===""?null:requiredString(v,"imageAssetId",100),null);take("imageAlt",v=>str(v,"imageAlt",240),"");
  return o;
}

/** Parses only the allowlisted, bounded JSON shape that may safely be persisted as a draft. */
export function parseEventDraft(value:Record<string,unknown>,partial=false){return parseEventShape(value,partial,false);}

/** Backwards-compatible strict parser for callers that validate complete event input. */
export function parseEvent(value:Record<string,unknown>,partial=false,draft=false){const o=parseEventShape(value,partial,!draft);if(!draft)validateDependencies(o,partial);return o;}
function validateDependencies(o: EventInput, partial: boolean) {
  ...
  if (["paid", "custom"].includes(String(o.costType)) && !String(o.costLabel ?? "").trim()) {
    throw validation("costLabel", "Enter the cost wording guests should see.");
  }
...
}export function materializePublishedTimes(o:EventInput){if(!o.eventDate)return {startsAt:0,endsAt:1,registrationOpensAt:null,registrationClosesAt:null};const d=parseDate(o.eventDate,"eventDate"),zone=String(o.timezone||"America/Boise");o.eventDate=d.text;if(!o.allDay&&(!o.startTime||!o.endTime))return {startsAt:0,endsAt:1,registrationOpensAt:null,registrationClosesAt:null};let startsAt:number,endsAt:number;if(o.allDay){startsAt=zonedInstant(d,{hour:0,minute:0},zone);const next=new Date(Date.UTC(d.year,d.month-1,d.day+1));endsAt=zonedInstant({text:"",year:next.getUTCFullYear(),month:next.getUTCMonth()+1,day:next.getUTCDate()},{hour:0,minute:0},zone);}else{const start=parseTime(o.startTime,"startTime"),end=parseTime(o.endTime,"endTime");o.startTime=start.text;o.endTime=end.text;startsAt=zonedInstant(d,start,zone,"startTime");endsAt=zonedInstant(d,end,zone,"endTime");if(endsAt<=startsAt)throw validation("endTime","End time must be after start time.");}const window=(prefix:"registrationOpens"|"registrationCloses")=>{const dateKey=`${prefix}Date`,timeKey=`${prefix}Time`,dv=o[dateKey],tv=o[timeKey];if(!dv&&!tv)return null;if(!dv||!tv)throw validation(`${prefix}${!dv?"Date":"Time"}`,"Enter both a date and time.");const parsedDate=parseDate(dv,dateKey),parsedTime=parseTime(tv,timeKey);o[dateKey]=parsedDate.text;o[timeKey]=parsedTime.text;return zonedInstant(parsedDate,parsedTime,zone,timeKey);};const registrationOpensAt=window("registrationOpens"),registrationClosesAt=window("registrationCloses");if(registrationOpensAt!==null&&registrationClosesAt!==null&&registrationClosesAt<=registrationOpensAt)throw validation("registrationClosesTime","Registration must close after it opens.");if(registrationClosesAt!==null&&registrationClosesAt>startsAt)throw validation("registrationClosesTime","Registration must close no later than the event start.");return {startsAt,endsAt,registrationOpensAt,registrationClosesAt};}

/** Drafts retain raw timing text; derived columns are populated only when every timing value is valid. */
export function materializeDraftTimes(o:EventInput){
  const neutral={startsAt:0,endsAt:1,registrationOpensAt:null,registrationClosesAt:null};
  const withoutWindow={...o,registrationOpensDate:"",registrationOpensTime:"",registrationClosesDate:"",registrationClosesTime:""};
  let eventTimes;
  try{eventTimes=materializePublishedTimes(withoutWindow);}catch{return neutral;}
  try{return materializePublishedTimes({...o});}catch{return {...eventTimes,registrationOpensAt:null,registrationClosesAt:null};}
}
export const materializeTimes=materializePublishedTimes;

export function validateEventForPublish(input:EventInput,now=Date.now()){const o=parseEventShape(input,false,true);const required:[string,string][]=[["title","Add an event title."],["shortDescription","Add a short card description."],["eventLabel","Choose an event label."],["eventDate","Add a valid event date."],["location","Add a location."],["attendanceType","Choose an attendance type."],["costType","Choose a cost type."],["imageAssetId","Add an event image."],["imageAlt","Add meaningful image alternative text."]];for(const [k,m] of required)if(!o[k])throw validation(k,m);if(!eventLabels.includes(o.eventLabel as typeof eventLabels[number]))throw validation("eventLabel","Choose a current event label before publishing.");if(!attendanceTypes.includes(o.attendanceType as typeof attendanceTypes[number]))throw validation("attendanceType","Choose a current attendance type before publishing.");if(String(o.imageAlt).trim().length<8)throw validation("imageAlt","Use meaningful alternative text of at least 8 characters.");if(!o.allDay&&(!o.startTime||!o.endTime))throw validation(!o.startTime?"startTime":"endTime","Add a valid time or select all day.");if(o.ctaAction!=="none"&&(!o.ctaAction||!o.ctaLabel))throw validation("ctaLabel","Add a CTA label and action.");validateDependencies(o,false);const times=materializePublishedTimes(o);if(times.endsAt<=now)throw validation("eventDate","The event has already ended.");return times;}
export const validateForPublish=validateEventForPublish;
export const iso=(v:number|null)=>v==null?null:new Date(v).toISOString();
export function eventJson(row:Record<string,unknown>){const v=(c:string,s:string)=>row[c]??row[s],b=(c:string,s:string,d=false)=>Boolean(v(c,s)??d);return {id:row.id,slug:row.slug??"",title:row.title??"",eventLabel:v("eventLabel","category")??"",customLabel:v("customLabel","custom_label")??"",shortDescription:v("shortDescription","short_description")??"",description:row.description??"",offer:row.offer??"",offerDetails:v("offerDetails","offer_details")??"",offerTerms:v("offerTerms","offer_terms")??"",eventDate:v("eventDate","event_date")??"",startTime:v("startTime","start_time")??"",endTime:v("endTime","end_time")??"",allDay:b("allDay","all_day"),timezone:row.timezone??"America/Boise",location:row.location??"",locationDetails:v("locationDetails","location_details")??"",directionsUrl:v("directionsUrl","directions_url")??"",attendanceType:v("attendanceType","attendance_type")??"",capacity:Number(row.capacity)>0?row.capacity:null,unlimitedCapacity:b("unlimitedCapacity","unlimited_capacity"),maxGuests:v("maxGuests","max_guests")??0,allowGuestNames:b("allowGuestNames","allow_guest_names"),registrationOpensDate:v("registrationOpensDate","registration_opens_date")??"",registrationOpensTime:v("registrationOpensTime","registration_opens_time")??"",registrationClosesDate:v("registrationClosesDate","registration_closes_date")??"",registrationClosesTime:v("registrationClosesTime","registration_closes_time")??"",allowDuplicateRegistration:b("allowDuplicateRegistration","allow_duplicate_registration"),appointmentRequired:b("appointmentRequired","appointment_required"),appointmentRecommended:b("appointmentRecommended","appointment_recommended"),costType:v("costType","cost_type")??"",costLabel:v("costLabel","cost_label")??"",ctaLabel:v("ctaLabel","cta_label")??"",ctaAction:v("ctaAction","cta_action")??"",ctaUrl:v("ctaUrl","cta_url")??"",ctaEmail:v("ctaEmail","cta_email")??"",ctaPhone:v("ctaPhone","cta_phone")??"",sharingEnabled:b("sharingEnabled","sharing_enabled",true),shareMessage:v("shareMessage","share_message")??"",status:row.status,startsAt:iso(Number(v("startsAt","starts_at"))),endsAt:iso(Number(v("endsAt","ends_at"))),publishedAt:iso(v("publishedAt","published_at") as number|null),archivedAt:iso(v("archivedAt","archived_at") as number|null),createdAt:iso(v("createdAt","created_at") as number),updatedAt:iso(v("updatedAt","updated_at") as number),imageAssetId:v("imageAssetId","image_asset_id")??null,imageMimeType:v("imageMimeType","image_mime_type")??null,imageSizeBytes:v("imageSizeBytes","image_size_bytes")??null,imageWidth:v("imageWidth","image_width")??null,imageHeight:v("imageHeight","image_height")??null,imageAlt:v("imageAlt","image_alt")??""};}
