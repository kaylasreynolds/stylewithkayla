...
export async function loadRecapSummarySource(db: D1Database, bookingId: string) {
  ...
  const [insights, items, formulas, priorities] = await Promise.all([
    db.prepare(`SELECT category,insight_text AS insightText,client_facing AS clientFacing FROM recap_insights WHERE recap_id=? ORDER BY sort_order`).bind(base.recapId).all<Row>(),
