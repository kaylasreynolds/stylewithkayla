...
export type ClientActionAccess={tokenId:string;bookingId:string;expiresAt:number};
export type ClientActionLookup=ClientActionAccess&{revokedAt:number|null};
export async function findClientActionAccess(raw:string){if(raw.length<40||raw.length>100)throw hidden();const hash=await hashPrivateToken(raw),row=await getD1().prepare(`SELECT id AS tokenId,booking_id AS bookingId,expires_at AS expiresAt,revoked_at AS revokedAt FROM private_access_tokens WHERE token_hash=? AND purpose='alternate_time' LIMIT 1`).bind(hash).first<ClientActionLookup>();if(!row)throw hidden();if(row.expiresAt<=Date.now())throw new ApiError(410,"PRIVATE_LINK_EXPIRED","This alternate-time link has expired. Please contact Kayla.");return row;}
