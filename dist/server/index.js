globalThis.__VINEXT_LAZY_CHUNKS__ = ["assets/link-BSWc2vms.js","assets/AdminDashboard-rvcoal74.js","assets/AdminAvailability-DPf7Y8Q8.js","assets/AdminPrivacy-CzgOTEcr.js","assets/page-kOzzFiZv.js","assets/page-Dn9cabTe.js","assets/page-B0m-hOUG.js","assets/layout-segment-context-DvKot9jq.js","assets/router-427wVbXz.js"];
...
function getDraftSecret() {
	return "6bda8ff3-5873-4cf2-a5ec-56215be9bfb0";
}
/**
...
var NextURL = class NextURL {
	...
	*/
	get buildId() {
		return "95c5a645-0de3-42fb-ae21-8bac57af26a9";
	}
};
...
*/
function appIsrCacheKey(pathname, suffix, buildId = "95c5a645-0de3-42fb-ae21-8bac57af26a9") {
...
function createAppPageArtifactCompatibility(element, routePattern) {
	...
	return createArtifactCompatibilityEnvelope({
		...
		deploymentVersion: "95c5a645-0de3-42fb-ae21-8bac57af26a9",
		rootBoundaryId
	});
}
/**
...
function dataResponse(data, status = 200, id = requestId()) {
...
function errorResponse(error, id = requestId()) {
	...
	return Response.json({
		error: {
			...
			...current.fieldErrors ? { fieldErrors: current.fieldErrors } : {}
		},
		requestId: id
	}, {
...
function rejectUnexpectedKeys(value, allowed, path = "request") {
...
function requiredString(value, field, max) {
	...
	if (result.length > max) throw validation(field, `Use ${max} characters or fewer.`);
	return result;
}
function optionalString(value, field, max) {
	return value === null || value === void 0 || value === "" ? null : requiredString(value, field, max);
}
function validation(field, message) {
	return new ApiError(422, "VALIDATION_ERROR", "Please check the highlighted fields.", { [field]: message });
}
//#endregion
//#region lib/server/runtime.ts
function getD1() {
	const db = env.DB;
...
function getAdminEmails() {
	return new Set((env.ADMIN_EMAILS?.split(",") ?? ["kaylasreynolds@gmail.com"]).map((v) => v.trim().toLowerCase()).filter(Boolean));
}
function getMaintenanceSecret() {
	return env.MAINTENANCE_SECRET?.trim() ?? "";
}
//#endregion
//#region lib/server/admin-auth.ts
function requireAdmin(request, mutation = false) {
	...
	if (mutation) {
		...
		if (!origin || origin !== new URL(request.url).origin) throw new ApiError(403, "CSRF_CHECK_FAILED", "Refresh the page and try again.");
	}
	return {
...
//#endregion
//#region app/api/admin/bookings/[bookingId]/export/route.ts
var route_exports$23 = /* @__PURE__ */ __exportAll({ GET: () => GET$12 });
async function GET$12(request, ctx) {
	requireAdmin(request);
	const { bookingId } = await ctx.params, db = getD1(), booking = await db.prepare(`SELECT b.*,c.full_name AS client_name,c.email AS client_email,c.phone AS client_phone,s.code AS service_code,s.name AS service_name,s.duration_minutes AS service_duration_minutes FROM bookings b JOIN clients c ON c.id=b.client_id JOIN services s ON s.id=b.service_id WHERE b.id=?`).bind(bookingId).first();
	if (!booking) return Response.json({ error: {
		code: "BOOKING_NOT_FOUND",
		message: "Booking request not found."
	} }, {
		status: 404,
		headers: privateHeaders$2()
	});
	const profile = await db.prepare(`SELECT id,profile_type,status,schema_version,answers,current_section,inspiration_link,submitted_at,locked_at,reopened_at,retention_delete_after,created_at,updated_at FROM style_profiles WHERE booking_id=?`).bind(bookingId).first(), history = (await db.prepare(`SELECT from_status,to_status,actor_type,reason,metadata,created_at FROM booking_status_history WHERE booking_id=? ORDER BY created_at`).bind(bookingId).all()).results, revisions = profile ? (await db.prepare(`SELECT revision_number,action,actor_type,note,changed_keys,answers_snapshot,created_at FROM style_profile_revisions WHERE profile_id=? ORDER BY revision_number`).bind(profile.id).all()).results : [], communications = (await db.prepare(`SELECT channel,template_key,recipient,status,metadata,sent_at,error_message,created_at FROM communications WHERE booking_id=? ORDER BY created_at`).bind(bookingId).all()).results;
	const payload = {
		exportedAt: (/* @__PURE__ */ new Date()).toISOString(),
		booking,
		profile: profile ? {
			...profile,
			answers: parse(profile.answers)
		} : null,
...
var geistSans = Geist({
	...
	subsets: ["latin"],
	_selfHostedCSS: "/* cyrillic-ext */\n@font-face {\n  font-family: 'Geist';\n  font-style: normal;\n  font-weight: 100 900;\n  font-display: swap;\n  src: url(/workspace/sites/style-with-kayla-booking-mockup/.vinext/fonts/geist-8ac0455e797f/geist-ff2310f5.woff2) format('woff2');\n  unicode-range: U+0460-052F, U+1C80-1C8A, U+20B4, U+2DE0-2DFF, U+A640-A69F, U+FE2E-FE2F;\n}\n/* cyrillic */\n@font-face {\n  font-family: 'Geist';\n  font-style: normal;\n  font-weight: 100 900;\n  font-display: swap;\n  src: url(/workspace/sites/style-with-kayla-booking-mockup/.vinext/fonts/geist-8ac0455e797f/geist-875ccdd4.woff2) format('woff2');\n  unicode-range: U+0301, U+0400-045F, U+0490-0491, U+04B0-04B1, U+2116;\n}\n/* vietnamese */\n@font-face {\n  font-family: 'Geist';\n  font-style: normal;\n  font-weight: 100 900;\n  font-display: swap;\n  src: url(/workspace/sites/style-with-kayla-booking-mockup/.vinext/fonts/geist-8ac0455e797f/geist-52306abf.woff2) format('woff2');\n  unicode-range: U+0102-0103, U+0110-0111, U+0128-0129, U+0168-0169, U+01A0-01A1, U+01AF-01B0, U+0300-0301, U+0303-0304, U+0308-0309, U+0323, U+0329, U+1EA0-1EF9, U+20AB;\n}\n/* latin-ext */\n@font-face {\n  font-family: 'Geist';\n  font-style: normal;\n  font-weight: 100 900;\n  font-display: swap;\n  src: url(/workspace/sites/style-with-kayla-booking-mockup/.vinext/fonts/geist-8ac0455e797f/geist-001175b1.woff2) format('woff2');\n  unicode-range: U+0100-02BA, U+02BD-02C5, U+02C7-02CC, U+02CE-02D7, U+02DD-02FF, U+0304, U+0308, U+0329, U+1D00-1DBF, U+1E00-1E9F, U+1EF2-1EFF, U+2020, U+20A0-20AB, U+20AD-20C0, U+2113, U+2C60-2C7F, U+A720-A7FF;\n}\n/* latin */\n@font-face {\n  font-family: 'Geist';\n  font-style: normal;\n  font-weight: 100 900;\n  font-display: swap;\n  src: url(/workspace/sites/style-with-kayla-booking-mockup/.vinext/fonts/geist-8ac0455e797f/geist-98bbbccb.woff2) format('woff2');\n  unicode-range: U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+0304, U+0308, U+0329, U+2000-206F, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD;\n}\n"
});
var geistMono = Geist_Mono({
	variable: "--font-geist-mono",
	subsets: ["latin"],
	_selfHostedCSS: "/* cyrillic-ext */\n@font-face {\n  font-family: 'Geist Mono';\n  font-style: normal;\n  font-weight: 100 900;\n  font-display: swap;\n  src: url(/workspace/sites/style-with-kayla-booking-mockup/.vinext/fonts/geist-mono-00e989178794/geist-mono-f6b33328.woff2) format('woff2');\n  unicode-range: U+0460-052F, U+1C80-1C8A, U+20B4, U+2DE0-2DFF, U+A640-A69F, U+FE2E-FE2F;\n}\n/* cyrillic */\n@font-face {\n  font-family: 'Geist Mono';\n  font-style: normal;\n  font-weight: 100 900;\n  font-display: swap;\n  src: url(/workspace/sites/style-with-kayla-booking-mockup/.vinext/fonts/geist-mono-00e989178794/geist-mono-44e03052.woff2) format('woff2');\n  unicode-range: U+0301, U+0400-045F, U+0490-0491, U+04B0-04B1, U+2116;\n}\n/* symbols2 */\n@font-face {\n  font-family: 'Geist Mono';\n  font-style: normal;\n  font-weight: 100 900;\n  font-display: swap;\n  src: url(/workspace/sites/style-with-kayla-booking-mockup/.vinext/fonts/geist-mono-00e989178794/geist-mono-0638449e.woff2) format('woff2');\n  unicode-range: U+2000-2001, U+2004-2008, U+200A, U+23B8-23BD, U+2500-259F;\n}\n/* vietnamese */\n@font-face {\n  font-family: 'Geist Mono';\n  font-style: normal;\n  font-weight: 100 900;\n  font-display: swap;\n  src: url(/workspace/sites/style-with-kayla-booking-mockup/.vinext/fonts/geist-mono-00e989178794/geist-mono-971fb274.woff2) format('woff2');\n  unicode-range: U+0102-0103, U+0110-0111, U+0128-0129, U+0168-0169, U+01A0-01A1, U+01AF-01B0, U+0300-0301, U+0303-0304, U+0308-0309, U+0323, U+0329, U+1EA0-1EF9, U+20AB;\n}\n/* latin-ext */\n@font-face {\n  font-family: 'Geist Mono';\n  font-style: normal;\n  font-weight: 100 900;\n  font-display: swap;\n  src: url(/workspace/sites/style-with-kayla-booking-mockup/.vinext/fonts/geist-mono-00e989178794/geist-mono-44745446.woff2) format('woff2');\n  unicode-range: U+0100-02BA, U+02BD-02C5, U+02C7-02CC, U+02CE-02D7, U+02DD-02FF, U+0304, U+0308, U+0329, U+1D00-1DBF, U+1E00-1E9F, U+1EF2-1EFF, U+2020, U+20A0-20AB, U+20AD-20C0, U+2113, U+2C60-2C7F, U+A720-A7FF;\n}\n/* latin */\n@font-face {\n  font-family: 'Geist Mono';\n  font-style: normal;\n  font-weight: 100 900;\n  font-display: swap;\n  src: url(/workspace/sites/style-with-kayla-booking-mockup/.vinext/fonts/geist-mono-00e989178794/geist-mono-013b2f2f.woff2) format('woff2');\n  unicode-range: U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+0304, U+0308, U+0329, U+2000-206F, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD;\n}\n"
});
var metadata = {
...
//#endregion
//#region lib/server/profile-policy.ts
var STYLE_PROFILE_TOKEN_TTL_MS = 720 * 60 * 60 * 1e3;
var PROFILE_TYPES = new Set([
	"under_40",
	"over_40",
	"womens_event",
	"mens_styling",
...
var shapes = [
	...
	"Wide Leg",
	"Open to trying different cuts",
	"Other"
];
var commonFinal = [q("changed_since_last_visit", "Have any of your sizes, preferences, or style changed since the last time we met?", "single", {
	options: [
		"Yes",
		"No",
		"Maybe"
	...
	returningOnly: true
}), q("additional_notes", "Is there anything else you'd like me to know before our appointment?", "long_text", { optional: true })];
var PROFILE_SCHEMAS = {
	under_40: [
		{
			title: "What you're looking for",
			intro: "This gives me a feel for the life your clothes need to support.",
			questions: [
				q("shopping_for", "What are you shopping for?", "multi", {
					required: true,
					maxSelections: 3,
					options: [
						"Everyday Outfits",
				...
				q("shopping_for_details", "Tell me a little more about what you need.", "long_text", { required: true }),
				q("top_styles", "Pick up to 3 styles that feel most like you.", "visual_multi", {
					required: true,
					maxSelections: 3,
					options: [
						"Minimal",
				...
				q("inspiration", "Do you have inspiration photos, a Pinterest board, or colors you want me to see?", "link", { optional: true }),
				q("feel_best_in", "What styles, colors, or silhouettes do you feel best in?", "long_text", { required: true }),
				q("avoid", "Are there any styles, colors, or pieces you do not want to wear?", "long_text", {
					required: true,
					placeholder: "Nothing specific is a valid answer."
				}),
				q("metal_preference", "Which metal do you usually prefer?", "single", {
					optional: true,
					options: [
						"Gold",
						"Silver",
		...
		},
		{
			...
			intro: "There are no right answers—this helps me pull better options.",
			questions: [
				q("body_shape", "Which illustration most closely resembles your shape?", "visual_single", {
					required: true,
					options: shapes
				}),
				q("frustration", "What's been most frustrating about getting dressed lately?", "single", {
					required: true,
					options: [
						"Nothing to wear",
						"Outfits don't feel put together",
						"Struggle with fit",
						"Don't know how to style what I have",
						"Closet feels outdated",
						"Other"
					]
				}),
				q("difficulty_fitting", "Which areas are usually most difficult to fit?", "multi", {
					required: true,
					options: [
						"Midsection",
						"Chest",
				...
				q("shop_sizing", "Which sizing department usually works best for you?", "single", {
					required: true,
					options: [
						"Regular",
						"Petite",
				...
				q("fabric_preference", "What tends to feel best on your body?", "single", {
					required: true,
					options: [
						"Structured",
						"Stretchy",
		...
		{
			title: "Sizes and silhouettes",
			intro: "Share your best estimate for a useful starting point.",
			questions: [
				q("top_size", "What top size do you usually wear?", "single", {
					required: true,
					options: [
						"XS",
						"S",
				...
				q("top_fit_preference", "How do you usually like your tops to fit?", "visual_multi", {
					required: true,
					options: topFits
				}),
				q("pant_size", "What pant size do you usually wear?", "text", {
					required: true,
					placeholder: "00/1–16/17, Not sure, or Other"
				}),
				q("pant_cut_preference", "Which pant cuts do you prefer?", "visual_multi", {
					required: true,
					options: pantCuts
				}),
				q("dress_size", "What dress size do you usually wear?", "text", {
					required: true,
					placeholder: "Size, Not sure, N/A, or Other"
				}),
				q("dress_styles", "Which dress styles do you usually feel best in?", "visual_multi", {
					options: [
						"Bodycon",
						"Shift",
						"A-Line",
				...
				q("shoe_size", "What shoe size do you usually wear?", "text", { required: true }),
				q("bra_size", "What bra size do you usually wear, if known?", "text", { optional: true })
			]
		},
		{
			title: "Final details",
			intro: "A last chance to share anything helpful.",
			questions: commonFinal
		}
	],
	over_40: [
		{
			title: "What you're looking for",
			intro: "Tell me what your wardrobe needs to support.",
			questions: [
				q("shopping_for", "What are you shopping for?", "multi", {
					required: true,
					maxSelections: 3,
				...
				q("shopping_for_details", "Tell me a little more about what you're looking for.", "long_text", { required: true }),
				q("top_styles", "Pick up to 3 styles that feel most like you.", "visual_multi", {
					required: true,
					maxSelections: 3,
					options: [
				...
				q("inspiration", "Do you have inspiration photos, a Pinterest board, or colors you want me to see?", "link", { optional: true }),
				q("feel_best_in", "What styles, colors, or pieces do you feel best in?", "long_text", { required: true }),
				q("avoid", "Are there any styles, colors, or pieces you do not want to wear?", "long_text", { required: true }),
				q("frustration", "What's been most frustrating about getting dressed lately?", "single", {
					required: true,
					options: [
						"Nothing to wear",
		...
		{
			title: "Fit and comfort",
			intro: "This helps me choose comfortable starting options.",
			questions: [
				q("body_shape", "Which illustration most closely resembles your shape?", "visual_single", {
					required: true,
					options: shapes
		...
		{
			title: "Sizes and silhouettes",
			intro: "Flexible estimates are welcome.",
			questions: [
				q("top_size", "What top size do you usually wear?", "text", { required: true }),
				q("top_fit_preference", "How do you usually like your tops to fit?", "visual_multi", {
					required: true,
					options: topFits
				}),
				q("neckline_preference", "Do you have a preferred neckline?", "single", {
					optional: true,
					options: [
						"Scoop",
						"Round",
				...
				q("pant_size", "What pant size do you usually wear?", "text", { required: true }),
				q("pant_cut_preference", "Which pant cuts do you prefer?", "visual_multi", {
					required: true,
					options: pantCuts
				}),
				q("dress_size", "What dress size do you usually wear?", "text", {
					required: true,
					placeholder: "Size, N/A, or Not sure"
				}),
				q("dress_styles", "Which dress styles do you usually feel best in?", "visual_multi", {
					options: [
						"Bodycon",
	...
	womens_event: [
		{
			title: "Your event",
			intro: "Start with the occasion and its dress expectations.",
			questions: [
				q("event_type", "What type of event are you shopping for?", "single", {
					required: true,
					options: [
						"Wedding Guest",
						"Wedding Party",
				...
				q("event_date", "When is the event?", "date", { required: true }),
				q("event_location", "Where is the event taking place? Is it indoors or outdoors?", "long_text", { required: true }),
				q("dress_code", "What is the dress code, if you know it?", "visual_single", {
					required: true,
					options: [
						"Casual",
						"Dressy Casual",
				...
				q("event_details", "Tell me a little more about the event.", "long_text", { optional: true }),
				q("required_colors_themes", "Are there required colors, themes, or dress guidelines?", "long_text", { optional: true })
			]
		},
		{
			title: "Styling goals",
			intro: "Choose the direction you want the event look to take.",
			questions: [
				q("top_event_styles", "Pick up to 3 styles for your event look.", "visual_multi", {
					required: true,
					maxSelections: 3,
					options: [
						"Classic Elegant",
		...
		{
			title: "Outfit preferences and fit",
			intro: "This helps narrow the right event options.",
			questions: [
				q("body_shape", "Which illustration most closely resembles your shape?", "visual_single", {
					required: true,
					options: shapes
				}),
				q("outfit_types_open_to", "Which outfit types are you open to?", "multi", {
					required: true,
					options: [
						"Dress",
						"Jumpsuit",
				...
				q("coverage_preference", "What level of coverage do you prefer?", "multi", {
					required: true,
					options: [
						"Sleeveless is fine",
						"Short sleeves",
				...
				q("fabric_preference", "What tends to feel best on your body?", "multi", {
					required: true,
					options: [
						"Structured",
						"Stretchy",
		...
		{
			title: "Sizes",
			intro: "Provide your best estimate for a starting point.",
			questions: [
				q("top_size", "What top size do you usually wear?", "text", { required: true }),
				q("top_fit_preference", "How do you usually like your tops to fit?", "visual_multi", {
					required: true,
				...
				q("dress_size", "What dress size do you usually wear?", "text", { required: true }),
				q("dress_styles", "Which dress styles do you usually feel best in?", "visual_multi", {
					options: [
						"Bodycon",
						"Fit and Flare",
				...
				q("shoe_size", "What shoe size do you usually wear?", "text", { required: true }),
				q("shoe_type_preference", "Which shoe types are you open to?", "multi", {
					required: true,
					options: [
						"High Heels 3+ inches",
						"Low Heels 2–3 inches",
						"Flats",
						"Dressy Sandals",
						"Boots / Booties",
						"Dressy Sneakers",
		...
		},
		{
			...
			intro: "A few final event details.",
			questions: [
				q("additional_help_interested", "What additional styling help would interest you?", "multi", {
					optional: true,
					options: [
						"Jewelry",
						"Handbag / Clutch",
				...
				q("shopping_for_others", "Are you shopping for anyone else for this event?", "single", {
					required: true,
					options: [
						"No",
						"Partner",
	...
	mens_styling: [{
		title: "What you're looking for",
		intro: "Tell me what your clothes need to support.",
		questions: [
			q("shopping_for", "What are you shopping for?", "multi", {
				required: true,
				maxSelections: 3,
			...
			q("typical_week", "What does a typical week look like for you?", "single", {
				required: true,
				options: [
					"Mostly Casual",
					"Business Casual",
			...
			q("feel_best_in", "Are there styles, colors, or pieces you feel best in?", "long_text", { required: true }),
			q("avoid", "Are there styles, colors, or pieces you do not want to wear?", "long_text", { required: true })
		]
	}, {
		title: "Fit and sizes",
		intro: "Flexible estimates are welcome.",
		questions: [
			...
			q("fit_preference", "What fit do you usually prefer?", "visual_single", {
				required: true,
				options: [
					"Slim",
					"Tailored",
			...
			q("shirt_size", "What shirt size do you usually wear?", "text", { required: true }),
			q("dress_shirt_size", "What dress-shirt size do you usually wear?", "text", {
				required: true,
				placeholder: "For example: 16 32/33, or Not sure"
			}),
			q("jacket_size", "What jacket size do you usually wear?", "text", {
				required: true,
				placeholder: "For example: 40R, or Not sure"
			}),
			q("pant_size", "What pant size do you usually wear?", "text", {
				required: true,
				placeholder: "For example: 36/34, or Not sure"
			}),
			q("shoe_size", "What shoe size do you usually wear?", "text", { required: true }),
			q("additional_notes", "Is there anything else you'd like me to know before our appointment?", "long_text", { optional: true })
		]
	...
	mens_event: [
		{
			title: "Your event",
			intro: "Start with the occasion and dress expectations.",
			questions: [
				q("event_type", "What type of event are you shopping for?", "single", {
					required: true,
					options: [
				...
				q("required_colors_themes", "Are there required colors, themes, or event guidelines?", "long_text", { optional: true })
			]
		},
		{
			title: "Style and outfit direction",
			intro: "Choose the direction for your event look.",
			questions: [
				q("top_event_styles", "Pick up to 3 styles for your event look.", "visual_multi", {
					required: true,
					maxSelections: 3,
...
function validateProfileDraft(value, submit = false, profileType = "under_40", returningClient = false) {
	if (!value || typeof value !== "object" || Array.isArray(value)) throw validation("answers", "Profile answers must be an object.");
	const answers = value, schema = PROFILE_SCHEMAS[profileType];
	if (!schema) throw new ApiError(422, "INVALID_PROFILE_TYPE", "This Style Profile type is not supported.");
	if (JSON.stringify(answers).length > 32e3) throw new ApiError(413, "PROFILE_TOO_LARGE", "The Style Profile is too large.");
	const questions = schema.flatMap((section) => section.questions), byKey = new Map(questions.map((question) => [question.key, question]));
	for (const key of Object.keys(answers)) if (!byKey.has(key)) throw validation(`answers.${key}`, "This profile field is not recognized.");
	for (const question of questions) {
		if (question.returningOnly && !returningClient) continue;
		if (question.showUnless && answers[question.showUnless.key] === question.showUnless.value) continue;
		const value = answers[question.key];
		if (value === void 0 || value === null || value === "" || Array.isArray(value) && !value.length) {
			if (submit && question.required) throw validation(`answers.${question.key}`, "This field is required.");
			continue;
		}
		if (question.kind === "multi" || question.kind === "visual_multi") {
			if (!Array.isArray(value) || value.some((item) => typeof item !== "string" || !item.trim() || item.length > 120)) throw validation(`answers.${question.key}`, "Choose valid options.");
			if (question.maxSelections && value.length > question.maxSelections) throw validation(`answers.${question.key}`, `Choose up to ${question.maxSelections} options.`);
			if (question.options && value.some((item) => !question.options.includes(item))) throw validation(`answers.${question.key}`, "Choose from the available options.");
		} else {
			if (typeof value !== "string" || value.length > 2e3) throw validation(`answers.${question.key}`, "This value is invalid or too long.");
			if ((question.kind === "single" || question.kind === "visual_single") && question.options && !question.options.includes(value)) throw validation(`answers.${question.key}`, "Choose from the available options.");
			if (question.kind === "date" && !/^\d{4}-\d{2}-\d{2}$/.test(value)) throw validation(`answers.${question.key}`, "Choose a valid date.");
		}
	}
	return answers;
}
//#endregion
//#region app/api/admin/bookings/[bookingId]/profile/route.ts
var route_exports$22 = /* @__PURE__ */ __exportAll({
	GET: () => GET$11,
	PATCH: () => PATCH$1
});
async function GET$11(request, ctx) {
	return withApi(async (id) => {
		requireAdmin(request);
		const { bookingId } = await ctx.params, db = getD1(), profile = await db.prepare(`SELECT p.id,p.profile_type AS profileType,p.status,p.schema_version AS schemaVersion,p.answers,p.current_section AS currentSection,p.inspiration_link AS inspirationLink,p.submitted_at AS submittedAt,p.reopened_at AS reopenedAt,b.returning_client AS returningClient FROM style_profiles p JOIN bookings b ON b.id=p.booking_id WHERE p.booking_id=?`).bind(bookingId).first();
		if (!profile) throw new ApiError(404, "PROFILE_NOT_FOUND", "This booking does not have a Style Profile yet.");
		for (const key of ["submittedAt", "reopenedAt"]) profile[key] = typeof profile[key] === "number" ? new Date(profile[key]).toISOString() : null;
		profile.returningClient = Boolean(profile.returningClient);
		profile.answers = typeof profile.answers === "string" ? JSON.parse(profile.answers) : profile.answers;
		return dataResponse({
			profile,
			revisions: (await db.prepare(`SELECT id,revision_number AS revisionNumber,action,actor_type AS actorType,reason,note,changed_keys AS changedKeys,created_at AS createdAt FROM style_profile_revisions WHERE profile_id=? ORDER BY revision_number DESC`).bind(profile.id).all()).results.map((row) => ({
				...row,
				changedKeys: typeof row.changedKeys === "string" ? JSON.parse(row.changedKeys) : row.changedKeys,
				createdAt: new Date(row.createdAt).toISOString()
			})),
			schema: PROFILE_SCHEMAS[String(profile.profileType)] ?? []
		}, 200, id);
	});
}
async function PATCH$1(request, ctx) {
	return withApi(async (id) => {
		const admin = requireAdmin(request, true), { bookingId } = await ctx.params, body = await readJsonObject(request);
		...
		if (!Array.isArray(body.changedKeys) || !body.changedKeys.length || body.changedKeys.some((key) => typeof key !== "string" || !key.trim())) throw validation("changedKeys", "List the fields being corrected.");
		const db = getD1(), profile = await db.prepare(`SELECT p.id,p.status,p.profile_type AS profileType,b.returning_client AS returningClient FROM style_profiles p JOIN bookings b ON b.id=p.booking_id WHERE p.booking_id=?`).bind(bookingId).first();
		if (!profile) throw new ApiError(404, "PROFILE_NOT_FOUND", "This booking does not have a Style Profile yet.");
		const answers = validateProfileDraft(body.answers, false, profile.profileType, Boolean(profile.returningClient)), now = Date.now();
		await db.batch([db.prepare(`UPDATE style_profiles SET answers=?,updated_at=? WHERE id=?`).bind(JSON.stringify(answers), now, profile.id), db.prepare(`INSERT INTO style_profile_revisions(id,profile_id,revision_number,action,actor_type,actor_id,answers_snapshot,changed_keys,note,created_at) VALUES(?,?,COALESCE((SELECT MAX(revision_number)+1 FROM style_profile_revisions WHERE profile_id=?),1),'admin_corrected','admin',?,?,?,?,?)`).bind(crypto.randomUUID(), profile.id, profile.id, admin.subjectId, JSON.stringify(answers), JSON.stringify(body.changedKeys), reason, now)]);
		return dataResponse({
...
//#endregion
//#region lib/server/crypto.ts
async function sha256(value) {
	const digest = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(value));
	return Array.from(new Uint8Array(digest), (b) => b.toString(16).padStart(2, "0")).join("");
}
async function hashPrivateToken(rawToken) {
	return sha256(rawToken);
}
function randomPrivateToken() {
	const bytes = crypto.getRandomValues(new Uint8Array(32));
	let binary = "";
	for (const byte of bytes) binary += String.fromCharCode(byte);
	return btoa(binary).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/g, "");
}
function randomPublicReference() {
	const alphabet = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789";
	return `SWK-${Array.from(crypto.getRandomValues(new Uint8Array(8)), (b) => alphabet[b % 32]).join("")}`;
}
//#endregion
//#region app/api/admin/bookings/[bookingId]/reopen-profile/route.ts
var route_exports$21 = /* @__PURE__ */ __exportAll({ POST: () => POST$10 });
async function POST$10(request, ctx) {
	return withApi(async (id) => {
		const admin = requireAdmin(request, true), { bookingId } = await ctx.params, body = await readJsonObject(request);
		rejectUnexpectedKeys(body, ["reason"]);
		const reason = requiredString(body.reason, "reason", 500), db = getD1(), profile = await db.prepare(`SELECT p.id,p.status,p.answers FROM style_profiles p JOIN bookings b ON b.id=p.booking_id WHERE p.booking_id=? AND b.status='confirmed'`).bind(bookingId).first();
		if (!profile) throw new ApiError(404, "PROFILE_NOT_FOUND", "This confirmed booking does not have a Style Profile.");
		const raw = randomPrivateToken(), hash = await hashPrivateToken(raw), now = Date.now(), expires = now + STYLE_PROFILE_TOKEN_TTL_MS;
		await db.batch([
			db.prepare(`UPDATE style_profiles SET status='reopened',locked_at=NULL,reopened_at=?,updated_at=? WHERE id=?`).bind(now, now, profile.id),
			db.prepare(`UPDATE private_access_tokens SET revoked_at=? WHERE profile_id=? AND purpose='style_profile' AND revoked_at IS NULL`).bind(now, profile.id),
			db.prepare(`INSERT INTO private_access_tokens(id,booking_id,profile_id,purpose,token_hash,expires_at,created_at) VALUES(?,?,?,'style_profile',?,?,?)`).bind(crypto.randomUUID(), bookingId, profile.id, hash, expires, now),
			db.prepare(`INSERT INTO style_profile_revisions(id,profile_id,revision_number,action,actor_type,actor_id,answers_snapshot,note,created_at) VALUES(?,?,COALESCE((SELECT MAX(revision_number)+1 FROM style_profile_revisions WHERE profile_id=?),1),'reopened','admin',?,?,?,?)`).bind(crypto.randomUUID(), profile.id, profile.id, admin.subjectId, profile.answers, reason, now)
		]);
		return dataResponse({
			status: "reopened",
...
//#endregion
//#region app/api/admin/bookings/[bookingId]/select-profile-type/route.ts
var route_exports$20 = /* @__PURE__ */ __exportAll({ POST: () => POST$9 });
async function POST$9(request, ctx) {
	return withApi(async (id) => {
		const admin = requireAdmin(request, true), { bookingId } = await ctx.params, body = await readJsonObject(request);
		rejectUnexpectedKeys(body, [
			"expectedStatus",
		...
		const expected = requiredString(body.expectedStatus, "expectedStatus", 40), profileType = requiredString(body.profileType, "profileType", 40), reason = requiredString(body.reason, "reason", 500);
		if (!PROFILE_TYPES.has(profileType)) throw new ApiError(422, "INVALID_PROFILE_TYPE", "Choose a valid Style Profile type.");
		const db = getD1(), now = Date.now(), booking = await db.prepare(`SELECT status,age_range AS ageRange,profile_type AS profileType FROM bookings WHERE id=?`).bind(bookingId).first();
		if (!booking) throw new ApiError(404, "BOOKING_NOT_FOUND", "Booking request not found.");
		if (booking.status !== expected) throw new ApiError(409, "BOOKING_STATE_CHANGED", "This request changed. Refresh before continuing.");
		if (!new Set(["pending", "change_proposed"]).has(expected)) throw new ApiError(409, "PROFILE_ROUTE_LOCKED", "Profile routing is locked after confirmation.");
		if (booking.ageRange !== "manual_review" && !booking.profileType) throw new ApiError(422, "PROFILE_ROUTE_NOT_MANUAL", "This booking does not require manual profile routing.");
		if (!(await db.prepare(`UPDATE bookings SET profile_type=?,updated_at=? WHERE id=? AND status=?`).bind(profileType, now, bookingId, expected).run()).meta.changes) throw new ApiError(409, "BOOKING_STATE_CHANGED", "This request changed. Refresh before continuing.");
		await db.prepare(`INSERT INTO booking_status_history(id,booking_id,from_status,to_status,actor_type,actor_id,reason,metadata,created_at) VALUES(?,?,?,?,'admin',?,?,?,?)`).bind(crypto.randomUUID(), bookingId, expected, expected, admin.subjectId, reason, JSON.stringify({
...
//#endregion
//#region app/api/admin/data-requests/[requestId]/export/route.ts
var route_exports$19 = /* @__PURE__ */ __exportAll({ GET: () => GET$10 });
var privateHeaders$1 = {
	"Cache-Control": "no-store",
	"Referrer-Policy": "no-referrer"
};
async function GET$10(request, ctx) {
	return withApi(async () => {
		requireAdmin(request);
		const { requestId } = await ctx.params, db = getD1();
		const record = await db.prepare(`SELECT dr.id,dr.kind,dr.status,dr.request_details AS requestDetails,dr.created_at AS requestedAt,c.id AS clientId,c.full_name AS fullName,c.email,c.phone,c.created_at AS clientCreatedAt,c.last_completed_appointment_at AS lastCompletedAppointmentAt,c.retention_delete_after AS retentionDeleteAfter FROM data_requests dr JOIN clients c ON c.id=dr.client_id WHERE dr.id=?`).bind(requestId).first();
		...
		const payload = {
			...
			client: {
				...
				email: record.email,
				phone: record.phone,
				createdAt: new Date(record.clientCreatedAt).toISOString(),
				lastCompletedAppointmentAt: record.lastCompletedAppointmentAt ? new Date(record.lastCompletedAppointmentAt).toISOString() : null,
				retentionDeleteAfter: record.retentionDeleteAfter ? new Date(record.retentionDeleteAfter).toISOString() : null
			},
			bookings,
			styleProfiles: profiles.map((profile) => ({
				...profile,
				answers: typeof profile.answers === "string" ? JSON.parse(profile.answers) : profile.answers
			}))
		};
		return new Response(JSON.stringify(payload, null, 2), { headers: {
			...privateHeaders$1,
			"Content-Type": "application/json; charset=utf-8",
			"Content-Disposition": `attachment; filename="style-with-kayla-data-request-${requestId}.json"`
		} });
	});
}
//#endregion
//#region lib/server/profile-access.ts
async function requireProfileAccess(raw) {
	if (raw.length < 40 || raw.length > 100) throw hidden();
	const hash = await hashPrivateToken(raw), row = await getD1().prepare(`SELECT t.id AS tokenId,t.profile_id AS profileId,t.booking_id AS bookingId,t.expires_at AS expiresAt,t.revoked_at AS revokedAt FROM private_access_tokens t WHERE t.token_hash=? AND t.purpose='style_profile' LIMIT 1`).bind(hash).first();
	if (!row) throw hidden();
	if (row.revokedAt || row.expiresAt <= Date.now()) throw new ApiError(410, "PRIVATE_LINK_EXPIRED", "This private link has expired. Please contact Kayla for a new link.");
	return row;
}
function hidden() {
	return new ApiError(404, "PRIVATE_LINK_NOT_FOUND", "This private link is not available.");
}
async function requireClientActionAccess(raw) {
	if (raw.length < 40 || raw.length > 100) throw hidden();
	const hash = await hashPrivateToken(raw), row = await getD1().prepare(`SELECT id AS tokenId,booking_id AS bookingId,expires_at AS expiresAt,revoked_at AS revokedAt FROM private_access_tokens WHERE token_hash=? AND purpose='alternate_time' LIMIT 1`).bind(hash).first();
	if (!row) throw hidden();
	if (row.revokedAt || row.expiresAt <= Date.now()) throw new ApiError(410, "PRIVATE_LINK_EXPIRED", "This alternate-time link has expired. Please contact Kayla.");
	return row;
}
//#endregion
//#region app/api/client-actions/[token]/accept-proposed-time/route.ts
var route_exports$18 = /* @__PURE__ */ __exportAll({ POST: () => POST$8 });
async function POST$8(request, ctx) {
	return withApi(async (id) => {
		const { token } = await ctx.params, access = await requireClientActionAccess(token), db = getD1(), booking = await db.prepare(`SELECT b.status,b.client_id AS clientId,b.profile_type AS profileType,b.proposed_start_at AS startsAt,b.proposed_end_at AS endsAt,c.email,h.id AS holdId FROM bookings b JOIN clients c ON c.id=b.client_id LEFT JOIN booking_holds h ON h.booking_id=b.id AND h.active=1 AND h.kind='proposed' WHERE b.id=?`).bind(access.bookingId).first();
		...
		const rawProfileToken = randomPrivateToken(), profileHash = await hashPrivateToken(rawProfileToken), now = Date.now(), expires = now + STYLE_PROFILE_TOKEN_TTL_MS;
		await db.batch([
			...
			db.prepare(`INSERT INTO private_access_tokens(id,booking_id,profile_id,purpose,token_hash,expires_at,created_at) SELECT ?,p.booking_id,p.id,'style_profile',?,?,? FROM style_profiles p WHERE p.booking_id=?`).bind(crypto.randomUUID(), profileHash, expires, now, access.bookingId),
			db.prepare(`INSERT INTO booking_status_history(id,booking_id,from_status,to_status,actor_type,metadata,created_at) SELECT ?,id,'change_proposed','confirmed','client',?,? FROM bookings WHERE id=? AND status='confirmed'`).bind(crypto.randomUUID(), JSON.stringify({
				action: "accept_proposed_time",
				tokenId: access.tokenId,
				profileLinkExpiresAt: new Date(expires).toISOString()
			}), now, access.bookingId),
			db.prepare(`INSERT INTO communications(id,booking_id,channel,template_key,recipient,status,metadata,created_at) SELECT ?,id,'email','booking_confirmed',?,'recorded',?,? FROM bookings WHERE id=? AND status='confirmed'`).bind(crypto.randomUUID(), booking.email, JSON.stringify({ deliveryDeferred: true }), now, access.bookingId)
		]);
		if ((await db.prepare(`SELECT status FROM bookings WHERE id=?`).bind(access.bookingId).first())?.status !== "confirmed") throw new ApiError(409, "ACTION_NO_LONGER_AVAILABLE", "This alternate-time request changed. Please contact Kayla.");
		return dataResponse({
			status: "confirmed",
			profileAccessUrl: `${new URL(request.url).origin}/style-profile/${rawProfileToken}`,
			profileAccessExpiresAt: new Date(expires).toISOString()
		}, 200, id);
	});
}
//#endregion
//#region lib/server/scheduling.ts
async function getActiveService(db, code) {
	const row = await db.prepare(`SELECT id,code,audience,name,duration_minutes AS durationMinutes,routing_mode AS routingMode FROM services WHERE code=? AND active=1`).bind(code).first();
	if (!row) throw new ApiError(404, "SERVICE_NOT_FOUND", "That styling service is not available.");
	return row;
}
async function calculateAvailability(db, code, from, to, now = Date.now(), excludeBookingId) {
	const service = await getActiveService(db, code);
	const settings = await db.prepare(`SELECT timezone,minimum_notice_minutes AS minimumNoticeMinutes,booking_horizon_days AS bookingHorizonDays FROM booking_settings WHERE id='default'`).first();
	if (!settings) throw new Error("Booking settings are missing.");
	validDate(from, "from");
	validDate(to, "to");
	if (from > to) throw new ApiError(422, "INVALID_DATE_RANGE", "The end date must be on or after the start date.");
	if (to > addDays(localDateKey(now, settings.timezone), settings.bookingHorizonDays) || daysBetween(from, to) > settings.bookingHorizonDays) throw new ApiError(422, "DATE_OUTSIDE_HORIZON", "Choose a date within the next 60 days.");
	const rangeStart = zonedLocalToEpoch(from, 0, settings.timezone), rangeEnd = zonedLocalToEpoch(addDays(to, 1), 0, settings.timezone);
	const rules = (await db.prepare(`SELECT weekday,start_minute AS startMinute,end_minute AS endMinute FROM availability_rules WHERE active=1 ORDER BY weekday,start_minute`).all()).results;
	const overrides = (await db.prepare(`SELECT kind,starts_at AS startsAt,ends_at AS endsAt FROM availability_overrides WHERE ends_at>? AND starts_at<?`).bind(rangeStart, rangeEnd).all()).results;
	...
	const holds = (await db.prepare(sql).bind(rangeStart, rangeEnd, ...excludeBookingId ? [excludeBookingId] : []).all()).results;
	const windows = [];
	for (let date = from; date <= to; date = addDays(date, 1)) {
		const weekday = weekdayForDate(date);
		for (const r of rules.filter((x) => x.weekday === weekday)) windows.push({
			startsAt: zonedLocalToEpoch(date, r.startMinute, settings.timezone),
			endsAt: zonedLocalToEpoch(date, r.endMinute, settings.timezone)
		});
	}
	windows.push(...overrides.filter((x) => x.kind === "available").map(({ startsAt, endsAt }) => ({
		startsAt,
		endsAt
	})));
	const unavailable = overrides.filter((x) => x.kind === "unavailable"), min = now + settings.minimumNoticeMinutes * 6e4, duration = service.durationMinutes * 6e4, candidates = /* @__PURE__ */ new Map();
	for (const w of windows) for (let start = w.startsAt; start + duration <= w.endsAt; start += duration) {
		const end = start + duration;
		if (start < rangeStart || start >= rangeEnd || start < min || unavailable.some((x) => overlaps(start, end, x)) || holds.some((x) => overlaps(start, end, x))) continue;
		candidates.set(start, end);
	}
	return {
		timezone: settings.timezone,
		availabilityMode: "routine_only",
		slots: Array.from(candidates, ([s, e]) => ({
			startsAt: s,
			endsAt: e
		})).sort((a, b) => a.startsAt - b.startsAt).map((x) => ({
...
function addDays(value, n) {
	...
	const [y, m, d] = value.split("-").map(Number), x = new Date(Date.UTC(y, m - 1, d + n));
	return `${x.getUTCFullYear()}-${pad(x.getUTCMonth() + 1)}-${pad(x.getUTCDate())}`;
}
function zonedLocalToEpoch(date, minuteOfDay, tz) {
	validDate(date, "date");
	const [y, m, d] = date.split("-").map(Number), h = Math.floor(minuteOfDay / 60), min = minuteOfDay % 60, desired = Date.UTC(y, m - 1, d, h, min);
	let epoch = desired;
	for (let i = 0; i < 4; i++) {
		const p = zonedParts(epoch, tz), correction = desired - Date.UTC(p.year, p.month - 1, p.day, p.hour, p.minute);
		epoch += correction;
		if (!correction) break;
	}
	return epoch;
...
function zonedParts(epoch, tz) {
	...
	for (const p of new Intl.DateTimeFormat("en-US", {
		...
		minute: "2-digit",
		hourCycle: "h23"
	}).formatToParts(epoch)) if (p.type !== "literal") v[p.type] = Number(p.value);
	return v;
}
function validDate(v, field) {
	if (!/^\d{4}-\d{2}-\d{2}$/.test(v)) throw new ApiError(422, "INVALID_DATE", `Use YYYY-MM-DD for ${field}.`);
	const [y, m, d] = v.split("-").map(Number), x = new Date(Date.UTC(y, m - 1, d));
	if (x.getUTCFullYear() !== y || x.getUTCMonth() + 1 !== m || x.getUTCDate() !== d) throw new ApiError(422, "INVALID_DATE", `Choose a valid ${field} date.`);
}
function weekdayForDate(v) {
...
function overlaps(s, e, w) {
	return w.startsAt < e && w.endsAt > s;
}
function pad(n) {
	return String(n).padStart(2, "0");
}
//#endregion
//#region app/api/client-actions/[token]/request-another-time/route.ts
var route_exports$17 = /* @__PURE__ */ __exportAll({ POST: () => POST$7 });
async function POST$7(request, ctx) {
	return withApi(async (id) => {
		const { token } = await ctx.params, access = await requireClientActionAccess(token), body = await readJsonObject(request);
		...
		const db = getD1(), booking = await db.prepare(`SELECT b.status,b.proposed_start_at AS proposedStartAt,b.proposed_end_at AS proposedEndAt,s.code AS serviceCode,c.email FROM bookings b JOIN services s ON s.id=b.service_id JOIN clients c ON c.id=b.client_id WHERE b.id=?`).bind(access.bookingId).first();
		...
		const date = localDateKey(start, "America/Boise"), slot = (await calculateAvailability(db, booking.serviceCode, date, date, Date.now(), access.bookingId)).slots.find((item) => item.startsAt === text);
		if (!slot) throw new ApiError(409, "SLOT_UNAVAILABLE", "That time is no longer available.");
		const end = Date.parse(slot.endsAt), holdId = crypto.randomUUID(), now = Date.now();
		await db.batch([
			...
			db.prepare(`UPDATE bookings SET status='pending',proposed_start_at=NULL,proposed_end_at=NULL,pending_since=?,updated_at=? WHERE id=? AND status='change_proposed' AND EXISTS(SELECT 1 FROM booking_holds WHERE id=? AND active=1)`).bind(now, now, access.bookingId, holdId),
			db.prepare(`UPDATE private_access_tokens SET revoked_at=? WHERE id=?`).bind(now, access.tokenId),
			db.prepare(`INSERT INTO booking_status_history(id,booking_id,from_status,to_status,actor_type,metadata,created_at) SELECT ?,id,'change_proposed','pending','client',?,? FROM bookings WHERE id=? AND status='pending'`).bind(crypto.randomUUID(), JSON.stringify({
				action: "request_another_time",
				priorProposedStartAt: booking.proposedStartAt ? new Date(booking.proposedStartAt).toISOString() : null,
				priorProposedEndAt: booking.proposedEndAt ? new Date(booking.proposedEndAt).toISOString() : null,
				newRequestedStartAt: text,
				newRequestedEndAt: slot.endsAt,
				tokenId: access.tokenId
			}), now, access.bookingId),
			db.prepare(`INSERT INTO communications(id,booking_id,channel,template_key,recipient,status,metadata,created_at) SELECT ?,id,'email','alternate_time_requested',?,'recorded',?,? FROM bookings WHERE id=? AND status='pending'`).bind(crypto.randomUUID(), booking.email, JSON.stringify({ deliveryDeferred: true }), now, access.bookingId)
		]);
		if ((await db.prepare(`SELECT status FROM bookings WHERE id=?`).bind(access.bookingId).first())?.status !== "pending") throw new ApiError(409, "SLOT_UNAVAILABLE", "That time is no longer available.");
		return dataResponse({
			status: "pending",
			requestedStartAt: text,
			requestedEndAt: slot.endsAt
		}, 200, id);
	});
}
//#endregion
//#region app/api/style-profile/[token]/calendar/route.ts
var route_exports$16 = /* @__PURE__ */ __exportAll({ GET: () => GET$9 });
async function GET$9(_request, ctx) {
	try {
		const { token } = await ctx.params, access = await requireProfileAccess(token), row = await getD1().prepare(`SELECT b.public_reference AS publicReference,b.confirmed_start_at AS startsAt,b.confirmed_end_at AS endsAt,s.name AS serviceName FROM bookings b JOIN services s ON s.id=b.service_id WHERE b.id=? AND b.status='confirmed'`).bind(access.bookingId).first();
		if (!row) return new Response("Calendar file unavailable.", {
			status: 404,
		...
		const ics = [
			"BEGIN:VCALENDAR",
			"VERSION:2.0",
			"PRODID:-//Style With Kayla//Appointment//EN",
			"CALSCALE:GREGORIAN",
			"METHOD:PUBLISH",
			"BEGIN:VEVENT",
			`UID:${escapeIcs(`${row.publicReference.toLowerCase()}@stylewithkayla.com`)}`,
			`DTSTAMP:${icsDate(Date.now())}`,
			`DTSTART:${icsDate(row.startsAt)}`,
			`DTEND:${icsDate(row.endsAt)}`,
			`SUMMARY:${escapeIcs(`${row.serviceName} with Kayla`)}`,
			`LOCATION:${escapeIcs("Macy's Boise Towne Square, 370 N. Milwaukee St., Boise, ID 83704")}`,
			`DESCRIPTION:${escapeIcs("Complimentary personal styling appointment with Kayla Reynolds.")}`,
			"END:VEVENT",
			"END:VCALENDAR",
			""
		].join("\r\n");
		return new Response(ics, {
			...
			headers: {
				...
				"Content-Disposition": `attachment; filename="style-with-kayla-${row.publicReference}.ics"`
			}
		});
	} catch {
...
//#endregion
//#region app/api/style-profile/[token]/submit/route.ts
var route_exports$15 = /* @__PURE__ */ __exportAll({ POST: () => POST$6 });
async function POST$6(_request, ctx) {
	return withApi(async (id) => {
		const { token } = await ctx.params, access = await requireProfileAccess(token), db = getD1(), profile = await db.prepare(`SELECT p.status,p.answers,p.profile_type AS profileType,b.returning_client AS returningClient FROM style_profiles p JOIN bookings b ON b.id=p.booking_id WHERE p.id=?`).bind(access.profileId).first();
		if (!profile) throw new Error("Style Profile is missing.");
		if (profile.status === "submitted") return dataResponse({ status: "submitted" }, 200, id);
		const answers = validateProfileDraft(typeof profile.answers === "string" ? JSON.parse(profile.answers) : profile.answers, true, profile.profileType, Boolean(profile.returningClient)), now = Date.now();
		await db.batch([db.prepare(`UPDATE style_profiles SET status='submitted',submitted_at=?,locked_at=?,updated_at=? WHERE id=? AND status IN ('draft','reopened')`).bind(now, now, now, access.profileId), db.prepare(`INSERT INTO style_profile_revisions(id,profile_id,revision_number,action,actor_type,answers_snapshot,changed_keys,created_at) SELECT ?,id,COALESCE((SELECT MAX(revision_number)+1 FROM style_profile_revisions WHERE profile_id=?),1),'submitted','client',?,NULL,? FROM style_profiles WHERE id=? AND status='submitted' AND NOT EXISTS(SELECT 1 FROM style_profile_revisions WHERE profile_id=? AND action='submitted' AND created_at=?)`).bind(crypto.randomUUID(), access.profileId, JSON.stringify(answers), now, access.profileId, access.profileId, now)]);
		return dataResponse({
			status: "submitted",
...
//#endregion
//#region app/api/admin/settings/availability/overrides/[overrideId]/route.ts
var route_exports$14 = /* @__PURE__ */ __exportAll({ DELETE: () => DELETE });
async function DELETE(request, ctx) {
	return withApi(async (id) => {
		requireAdmin(request, true);
		const { overrideId } = await ctx.params;
		if (!(await getD1().prepare(`DELETE FROM availability_overrides WHERE id=?`).bind(overrideId).run()).meta.changes) throw new ApiError(404, "OVERRIDE_NOT_FOUND", "Availability override not found.");
		return dataResponse({
			overrideId,
			deleted: true
		}, 200, id);
...
async function GET$8(request, ctx) {
	return withApi(async (id) => {
		...
		for (const f of [
			...
			"activeHoldStartsAt",
			"activeHoldEndsAt"
		]) b[f] = typeof b[f] === "number" ? new Date(b[f]).toISOString() : null;
		b.returningClient = Boolean(b.returningClient);
		return dataResponse({
			booking: b,
			history: (await db.prepare(`SELECT id,from_status AS fromStatus,to_status AS toStatus,actor_type AS actorType,reason,created_at AS createdAt FROM booking_status_history WHERE booking_id=? ORDER BY created_at DESC`).bind(bookingId).all()).results.map((x) => ({
				...x,
...
//#endregion
//#region lib/server/retention-policy.ts
function anonymizedClientEmail(clientId) {
	return `deleted+${clientId.toLowerCase().replace(/[^a-z0-9-]/g, "")}@invalid.local`;
}
//#endregion
//#region lib/server/privacy-operations.ts
var eligibleSql = `
...
async function maintenanceSummary(db, now = Date.now()) {
	const [expired, due, open] = await Promise.all([
		db.prepare(`SELECT COUNT(*) AS count FROM private_access_tokens WHERE revoked_at IS NULL AND expires_at <= ?`).bind(now).first(),
		db.prepare(`SELECT COUNT(*) AS count FROM clients c WHERE ${eligibleSql}`).bind(now).first(),
		db.prepare(`SELECT COUNT(*) AS count FROM data_requests WHERE status IN ('open','in_progress')`).first()
	]);
	return {
		expiredTokenCount: expired?.count ?? 0,
		dueClientCount: due?.count ?? 0,
		openDataRequestCount: open?.count ?? 0
	};
}
async function eraseClientData(db, clientId, now, auditRequestId, resolutionNote) {
...
async function runRetentionMaintenance(db, now = Date.now(), limit = 100) {
	...
	const candidates = (await db.prepare(`SELECT c.id,c.full_name AS fullName,c.email,c.retention_delete_after AS retentionDeleteAfter FROM clients c WHERE ${eligibleSql} ORDER BY c.retention_delete_after LIMIT ?`).bind(now, limit).all()).results;
	...
	return {
		revokedExpiredTokenCount: revoked.meta.changes ?? 0,
		deletedClientCount: candidates.length,
		limitReached: candidates.length === limit
	};
}
async function completeDeletionRequest(db, requestId, resolutionNote, now = Date.now()) {
	const request = await db.prepare(`SELECT id,client_id AS clientId,kind,status FROM data_requests WHERE id=?`).bind(requestId).first();
	...
	if (((await db.prepare(`SELECT COUNT(*) AS count FROM bookings WHERE client_id=? AND status IN ('pending','change_proposed','confirmed')`).bind(request.clientId).first())?.count ?? 0) > 0) throw new ApiError(409, "ACTIVE_BOOKING_PREVENTS_DELETION", "Decline or cancel active appointments before deleting this client’s written data.");
...
//#endregion
//#region app/api/admin/data-requests/[requestId]/route.ts
var route_exports$12 = /* @__PURE__ */ __exportAll({ PATCH: () => PATCH });
var statuses$2 = new Set([
	"in_progress",
	"completed",
	"declined"
]);
async function PATCH(request, ctx) {
	return withApi(async (id) => {
		...
		rejectUnexpectedKeys(body, ["status", "resolutionNote"]);
		const status = requiredString(body.status, "status", 20), note = typeof body.resolutionNote === "string" ? body.resolutionNote.trim() : "";
		if (!statuses$2.has(status)) throw new ApiError(422, "INVALID_DATA_REQUEST_STATUS", "Choose in progress, completed, or declined.");
		...
		const db = getD1(), current = await db.prepare(`SELECT id,kind,status FROM data_requests WHERE id=?`).bind(requestId).first();
		...
		const now = Date.now();
		if (!(await db.prepare(`UPDATE data_requests SET status=?,resolution_note=?,completed_at=? WHERE id=? AND status IN ('open','in_progress')`).bind(status, note || null, status === "in_progress" ? null : now, requestId).run()).meta.changes) throw new ApiError(409, "DATA_REQUEST_STATE_CHANGED", "This request changed while you were reviewing it.");
		return dataResponse({
			requestId,
			status,
			completedAt: status === "in_progress" ? null : new Date(now).toISOString()
		}, 200, id);
	});
}
//#endregion
...
async function AdminPage() {
	...
	if (!getAdminEmails().has(user.email.toLowerCase())) return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("main", {
		className: "admin-denied",
		children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h1", { children: "Access unavailable" }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "This account cannot manage appointment requests." })]
	});
...
var AdminAvailability_default = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'default' is called on server");
}, "4087ee207052", "default");
//#endregion
//#region app/admin/availability/page.tsx
...
async function AvailabilityPage() {
	const user = await requireChatGPTUser("/admin/availability");
	if (!getAdminEmails().has(user.email.toLowerCase())) return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("main", {
		className: "admin-denied",
		children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h1", { children: "Access unavailable" })
	});
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(AdminAvailability_default, {
		userName: user.displayName,
		signOutPath: chatGPTSignOutPath("/")
	});
}
...
var AdminPrivacy_default = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'default' is called on server");
}, "7ec147572b06", "default");
//#endregion
//#region app/admin/privacy/page.tsx
var page_exports$4 = /* @__PURE__ */ __exportAll({
	default: () => AdminPrivacyPage,
	dynamic: () => dynamic$2
});
var dynamic$2 = "force-dynamic";
async function AdminPrivacyPage() {
	const user = await requireChatGPTUser("/admin/privacy");
	if (!getAdminEmails().has(user.email.toLowerCase())) return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("main", {
		className: "admin-denied",
		children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h1", { children: "Access unavailable" }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "This account cannot manage privacy operations." })]
	});
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(AdminPrivacy_default, {
		userName: user.displayName,
		signOutPath: chatGPTSignOutPath("/")
	});
}
//#endregion
//#region app/api/admin/bookings/route.ts
var route_exports$11 = /* @__PURE__ */ __exportAll({ GET: () => GET$7 });
var statuses$1 = new Set([
	"pending",
	"change_proposed",
	"confirmed",
	"declined",
...
async function GET$7(request) {
	return withApi(async (id) => {
		...
		if (!statuses$1.has(status)) throw new ApiError(422, "INVALID_STATUS", "Choose a valid booking status.");
		const rows = (await getD1().prepare(`SELECT b.id,b.public_reference AS publicReference,b.status,b.pending_since AS pendingSince,b.requested_start_at AS requestedStartAt,b.requested_end_at AS requestedEndAt,b.proposed_start_at AS proposedStartAt,c.full_name AS clientName,c.email AS clientEmail,s.name AS serviceName,s.code AS serviceCode,CASE WHEN h.id IS NULL THEN 0 ELSE 1 END AS holdActive FROM bookings b JOIN clients c ON c.id=b.client_id JOIN services s ON s.id=b.service_id LEFT JOIN booking_holds h ON h.booking_id=b.id AND h.active=1 WHERE b.status=? ORDER BY b.pending_since ASC LIMIT 50`).bind(status).all()).results;
		return dataResponse({
			bookings: rows.map((r) => ({
				...r,
				pendingSince: new Date(r.pendingSince).toISOString(),
				requestedStartAt: new Date(r.requestedStartAt).toISOString(),
				requestedEndAt: new Date(r.requestedEndAt).toISOString(),
				proposedStartAt: r.proposedStartAt ? new Date(r.proposedStartAt).toISOString() : null,
				holdActive: Boolean(r.holdActive),
				overdue: Date.now() - r.pendingSince >= 864e5
			})),
			nextCursor: null
		}, 200, id);
	});
...
//#endregion
//#region app/api/admin/data-requests/route.ts
...
async function GET$6(request) {
	return withApi(async (id) => {
		...
		const rows = (status === "all" || status === "active" ? await statement.all() : await statement.bind(status).all()).results;
		return dataResponse({ requests: rows.map((row) => ({
			...row,
...
async function POST$5(request) {
	return withApi(async (id) => {
		...
		const email = requiredString(body.clientEmail, "clientEmail", 254).toLowerCase(), kind = requiredString(body.kind, "kind", 20), details = optionalString(body.requestDetails, "requestDetails", 2e3);
		if (!kinds.has(kind)) throw new ApiError(422, "INVALID_DATA_REQUEST_KIND", "Choose access, correction, or deletion.");
		const db = getD1(), client = await db.prepare(`SELECT id,full_name AS fullName,email FROM clients WHERE normalized_email=?`).bind(email).first();
		...
		await db.batch([db.prepare(`INSERT INTO data_requests(id,client_id,kind,status,request_details,created_at) VALUES(?,? ,?,'open',?,?)`).bind(requestId, client.id, kind, details, now), ...kind === "deletion" ? [db.prepare(`UPDATE clients SET deletion_requested_at=?,updated_at=? WHERE id=?`).bind(now, now, client.id)] : []]);
...
//#endregion
//#region app/api/admin/operations/maintenance/route.ts
...
async function GET$5(request) {
	return withApi(async (id) => {
		requireAdmin(request);
		return dataResponse(await maintenanceSummary(getD1()), 200, id);
	});
}
...
//#endregion
//#region lib/server/availability-policy.ts
function validateAvailabilityRules(value) {
	...
	const rules = value.map((item, index) => {
		if (!item || typeof item !== "object" || Array.isArray(item)) throw validation(`rules.${index}`, "Each rule must be an object.");
		const row = item;
		for (const key of Object.keys(row)) if (!new Set([
			"weekday",
			"startMinute",
			"endMinute"
		]).has(key)) throw new ApiError(400, "UNEXPECTED_FIELD", `rules.${index} contains an unexpected field: ${key}.`);
		const weekday = Number(row.weekday), startMinute = Number(row.startMinute), endMinute = Number(row.endMinute);
		if (!Number.isInteger(weekday) || weekday < 2 || weekday > 6) throw validation(`rules.${index}.weekday`, "Routine availability is limited to Tuesday through Saturday.");
		if (!Number.isInteger(startMinute) || !Number.isInteger(endMinute) || startMinute < 0 || endMinute > 1440 || startMinute >= endMinute) throw validation(`rules.${index}`, "Choose a valid start and end time.");
		return {
			weekday,
			startMinute,
			endMinute
	...
	for (const weekday of [
		...
		for (let index = 1; index < day.length; index++) if (day[index].startMinute < day[index - 1].endMinute) throw validation("rules", "Routine windows cannot overlap.");
	}
	return rules;
}
//#endregion
//#region app/api/admin/settings/availability/route.ts
...
async function GET$4(request) {
	return withApi(async (id) => {
		requireAdmin(request);
		const db = getD1(), settings = await db.prepare(`SELECT timezone,minimum_notice_minutes AS minimumNoticeMinutes,booking_horizon_days AS bookingHorizonDays,pending_overdue_minutes AS pendingOverdueMinutes,calendar_conflict_check_enabled AS calendarConflictCheckEnabled FROM booking_settings WHERE id='default'`).first(), rules = (await db.prepare(`SELECT id,weekday,start_minute AS startMinute,end_minute AS endMinute,timezone FROM availability_rules WHERE active=1 ORDER BY weekday,start_minute`).all()).results;
		return dataResponse({
			settings,
			rules,
			overrides: (await db.prepare(`SELECT id,kind,starts_at AS startsAt,ends_at AS endsAt,note,created_by AS createdBy,created_at AS createdAt FROM availability_overrides WHERE ends_at>? ORDER BY starts_at`).bind(Date.now() - 864e5).all()).results.map((row) => ({
				...row,
...
async function PUT$1(request) {
	return withApi(async (id) => {
		const admin = requireAdmin(request, true), body = await readJsonObject(request);
		rejectUnexpectedKeys(body, ["rules"]);
		const rules = validateAvailabilityRules(body.rules), db = getD1(), now = Date.now(), statements = [db.prepare(`UPDATE availability_rules SET active=0,updated_at=?`).bind(now), ...rules.map((rule, index) => db.prepare(`INSERT INTO availability_rules(id,weekday,start_minute,end_minute,timezone,active,created_at,updated_at) VALUES(?,?,?,?,'America/Boise',1,?,?) ON CONFLICT(weekday,start_minute,end_minute) DO UPDATE SET active=1,timezone='America/Boise',updated_at=excluded.updated_at`).bind(`rule_admin_${rule.weekday}_${rule.startMinute}_${rule.endMinute}_${index}`, rule.weekday, rule.startMinute, rule.endMinute, now, now))];
		await db.batch(statements);
		return dataResponse({
			rules,
			updatedBy: admin.subjectId,
			updatedAt: new Date(now).toISOString()
		}, 200, id);
	});
}
//#endregion
//#region app/api/admin/settings/availability/overrides/route.ts
var route_exports$7 = /* @__PURE__ */ __exportAll({ POST: () => POST$3 });
async function POST$3(request) {
	return withApi(async (id) => {
		...
		if (kind !== "available" && kind !== "unavailable") throw validation("kind", "Choose available or unavailable.");
		const startText = requiredString(body.startsAt, "startsAt", 40), endText = requiredString(body.endsAt, "endsAt", 40), startsAt = Date.parse(startText), endsAt = Date.parse(endText);
		if (!Number.isFinite(startsAt) || !Number.isFinite(endsAt) || startsAt >= endsAt || new Date(startsAt).toISOString() !== startText || new Date(endsAt).toISOString() !== endText) throw validation("startsAt", "Choose a valid start and end time.");
		if (endsAt - startsAt > 7 * 864e5) throw validation("endsAt", "An override cannot span more than seven days.");
		const note = body.note === void 0 || body.note === null || body.note === "" ? null : requiredString(body.note, "note", 500), db = getD1(), overrideId = crypto.randomUUID(), now = Date.now();
		await db.prepare(`INSERT INTO availability_overrides(id,kind,starts_at,ends_at,note,created_by,created_at) VALUES(?,?,?,?,?,?,?)`).bind(overrideId, kind, startsAt, endsAt, note, admin.subjectId, now).run();
		return dataResponse({
			overrideId,
			kind,
			startsAt: startText,
...
//#endregion
//#region app/api/availability/route.ts
var route_exports$6 = /* @__PURE__ */ __exportAll({ GET: () => GET$3 });
async function GET$3(request) {
	return withApi(async (id) => {
		const u = new URL(request.url), s = u.searchParams.get("serviceCode"), f = u.searchParams.get("from"), t = u.searchParams.get("to");
		if (!s || !f || !t) throw new ApiError(422, "MISSING_QUERY", "Choose a service and date range.");
		return dataResponse(await calculateAvailability(getD1(), s, f, t), 200, id);
	});
}
//#endregion
//#region app/api/bookings/route.ts
...
async function POST$2(request) {
	return withApi(async (requestId) => {
		...
		const client = obj(body.client, "client"), privacy = obj(body.privacy, "privacy");
		rejectUnexpectedKeys(client, [
			"fullName",
			"email",
			"phone"
		], "client");
		rejectUnexpectedKeys(privacy, ["policyVersion", "acknowledged"], "privacy");
		const fullName = requiredString(client.fullName, "client.fullName", 120), email = requiredString(client.email, "client.email", 254), normalizedEmail = email.toLowerCase();
		if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalizedEmail)) throw validation("client.email", "Enter a valid email address.");
		const phone = requiredString(client.phone, "client.phone", 40), normalizedPhone = phone.replace(/\D/g, "");
		if (normalizedPhone.length < 7 || normalizedPhone.length > 15) throw validation("client.phone", "Enter a valid phone number.");
		const serviceCode = requiredString(body.serviceCode, "serviceCode", 60), startText = requiredString(body.requestedStartAt, "requestedStartAt", 40), start = Date.parse(startText);
		if (!Number.isFinite(start) || new Date(start).toISOString() !== startText) throw validation("requestedStartAt", "Choose an available appointment time.");
		if (typeof body.returningClient !== "boolean") throw validation("returningClient", "Choose yes or no.");
		const returning = body.returningClient, howHeard = returning ? "Returning Client" : requiredString(body.howHeard, "howHeard", 80);
		if (!returning && !heard.has(howHeard)) throw validation("howHeard", "Choose one of the available options.");
		if (privacy.acknowledged !== true) throw validation("privacy.acknowledged", "Please acknowledge the privacy notice.");
		const policy = requiredString(privacy.policyVersion, "privacy.policyVersion", 40);
		const db = getD1(), service = await getActiveService(db, serviceCode);
		let ageRange = null, profileType = null, eventType = null, eventDate = null;
		if (service.routingMode === "age") {
			ageRange = requiredString(body.ageRange, "ageRange", 30);
			if (!ages.has(ageRange)) throw validation("ageRange", "Choose one of the available age ranges.");
			profileType = ageRange === "under_40" ? "under_40" : ageRange === "40_plus" ? "over_40" : null;
		} else if (service.routingMode === "womens_event" || service.routingMode === "mens_event") {
			eventType = requiredString(body.eventType, "eventType", 120);
			eventDate = requiredString(body.eventDate, "eventDate", 10);
			if (!/^\d{4}-\d{2}-\d{2}$/.test(eventDate)) throw validation("eventDate", "Choose a valid event date.");
			profileType = service.routingMode;
		} else profileType = "mens_styling";
		const notes = optionalString(body.bookingNotes, "bookingNotes", 2e3), date = localDateKey(start, "America/Boise"), slot = (await calculateAvailability(db, serviceCode, date, date)).slots.find((x) => x.startsAt === startText);
		if (!slot) throw new ApiError(409, "SLOT_UNAVAILABLE", "That appointment time is no longer available.", { requestedStartAt: "Choose another available time." });
		...
		const payloadHash = await sha256(JSON.stringify({
			...
			notes,
			policy
		})), key = request.headers.get("idempotency-key")?.trim() || crypto.randomUUID();
		if (key.length > 200) throw new ApiError(400, "INVALID_IDEMPOTENCY_KEY", "The idempotency key is too long.");
		const keyHash = await sha256(key);
		const existing = await findExisting(db, keyHash);
		if (existing) {
			if (existing.payloadHash !== payloadHash) throw new ApiError(409, "IDEMPOTENCY_KEY_REUSED", "This submission key was already used for a different request.");
			return response(existing, requestId);
		}
		const rateHash = await sha256(`${request.headers.get("cf-connecting-ip") ?? request.headers.get("x-forwarded-for")?.split(",")[0]?.trim() ?? "unknown"}\n${normalizedEmail}`);
		if (((await db.prepare(`SELECT COUNT(*) AS count FROM booking_status_history WHERE created_at>=? AND json_extract(metadata,'$.rateLimitKeyHash')=?`).bind(Date.now() - 36e5, rateHash).first())?.count ?? 0) >= 5) throw new ApiError(429, "RATE_LIMITED", "Too many requests were submitted. Please try again later.");
		const now = Date.now(), bookingId = crypto.randomUUID(), publicReference = randomPublicReference(), meta = JSON.stringify({
			idempotencyKeyHash: keyHash,
			payloadHash,
			rateLimitKeyHash: rateHash,
...
//#endregion
//#region app/api/services/route.ts
var route_exports$3 = /* @__PURE__ */ __exportAll({ GET: () => GET$2 });
async function GET$2() {
	return withApi(async (id) => dataResponse({ services: (await getD1().prepare(`SELECT code,audience,name,duration_minutes AS durationMinutes,routing_mode AS routingMode FROM services WHERE active=1 ORDER BY sort_order,name`).all()).results }, 200, id));
}
//#endregion
//#region app/client-actions/page.tsx
var page_exports$3 = /* @__PURE__ */ __exportAll({
	ClientActionClient: () => ClientActionClient,
	default: () => page_default$1
});
var page_default$1 = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'default' is called on server");
}, "b68f4ee01005", "default");
var ClientActionClient = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'ClientActionClient' is called on server");
}, "b68f4ee01005", "ClientActionClient");
//#endregion
//#region app/style-profile/page.tsx
var page_exports$2 = /* @__PURE__ */ __exportAll({
	StyleProfileClient: () => StyleProfileClient,
	default: () => page_default
});
var page_default = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'default' is called on server");
}, "fc858ccfd676", "default");
var StyleProfileClient = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'StyleProfileClient' is called on server");
}, "fc858ccfd676", "StyleProfileClient");
//#endregion
//#region app/api/client-actions/[token]/route.ts
var route_exports$2 = /* @__PURE__ */ __exportAll({ GET: () => GET$1 });
async function GET$1(_request, ctx) {
	return withApi(async (id) => {
		const { token } = await ctx.params, access = await requireClientActionAccess(token), row = await getD1().prepare(`SELECT b.status,b.public_reference AS publicReference,b.requested_start_at AS requestedStartAt,b.proposed_start_at AS proposedStartAt,b.proposed_end_at AS proposedEndAt,s.code AS serviceCode,s.name AS serviceName,c.full_name AS clientName FROM bookings b JOIN services s ON s.id=b.service_id JOIN clients c ON c.id=b.client_id WHERE b.id=?`).bind(access.bookingId).first();
		if (!row || row.status !== "change_proposed") throw new ApiError(409, "ACTION_NO_LONGER_AVAILABLE", "This alternate-time request is no longer active.");
		return dataResponse({
			booking: {
				...row,
				clientFirstName: String(row.clientName).trim().split(/\s+/)[0],
				clientName: void 0,
				requestedStartAt: new Date(row.requestedStartAt).toISOString(),
				proposedStartAt: new Date(row.proposedStartAt).toISOString(),
				proposedEndAt: new Date(row.proposedEndAt).toISOString()
			},
...
//#endregion
//#region app/api/style-profile/[token]/route.ts
...
async function GET(_request, ctx) {
	return withApi(async (id) => {
		const { token } = await ctx.params, access = await requireProfileAccess(token), row = await getD1().prepare(`SELECT p.status,p.profile_type AS profileType,p.schema_version AS schemaVersion,p.answers,p.current_section AS currentSection,p.inspiration_link AS inspirationLink,c.full_name AS clientName,s.name AS serviceName,b.confirmed_start_at AS confirmedStartAt,b.confirmed_end_at AS confirmedEndAt,b.returning_client AS returningClient,b.event_type AS eventType,b.event_date AS eventDate FROM style_profiles p JOIN bookings b ON b.id=p.booking_id JOIN clients c ON c.id=p.client_id JOIN services s ON s.id=b.service_id WHERE p.id=? AND b.status='confirmed'`).bind(access.profileId).first();
		if (!row) throw validation("profile", "This Style Profile is not available.");
		const profileType = String(row.profileType), schema = PROFILE_SCHEMAS[profileType];
		if (!schema) throw validation("profile", "This Style Profile type is not available.");
		const answers = typeof row.answers === "string" ? JSON.parse(row.answers) : row.answers;
		if ((profileType === "womens_event" || profileType === "mens_event") && !answers.event_type && row.eventType) answers.event_type = row.eventType;
		if ((profileType === "womens_event" || profileType === "mens_event") && !answers.event_date && row.eventDate) answers.event_date = row.eventDate;
		return dataResponse({
			profile: {
				...row,
				returningClient: Boolean(row.returningClient),
				clientFirstName: String(row.clientName).trim().split(/\s+/)[0],
				clientName: void 0,
				answers,
				confirmedStartAt: new Date(row.confirmedStartAt).toISOString(),
				confirmedEndAt: new Date(row.confirmedEndAt).toISOString(),
				schema
			},
			expiresAt: new Date(access.expiresAt).toISOString()
		}, 200, id);
...
async function PUT(request, ctx) {
	return withApi(async (id) => {
		const { token } = await ctx.params, access = await requireProfileAccess(token), body = await readJsonObject(request);
		rejectUnexpectedKeys(body, [
			"schemaVersion",
			"currentSection",
			"answers",
		...
		if (body.schemaVersion !== 1) throw validation("schemaVersion", "Refresh this page before saving.");
		const db = getD1(), profile = await db.prepare(`SELECT p.profile_type AS profileType,b.returning_client AS returningClient FROM style_profiles p JOIN bookings b ON b.id=p.booking_id WHERE p.id=?`).bind(access.profileId).first();
		if (!profile) throw validation("profile", "This Style Profile is not available.");
		const sectionCount = PROFILE_SCHEMAS[profile.profileType]?.length ?? 0;
		if (!Number.isInteger(body.currentSection) || body.currentSection < 1 || body.currentSection > sectionCount) throw validation("currentSection", "Choose a valid section.");
		const answers = validateProfileDraft(body.answers, false, profile.profileType, Boolean(profile.returningClient)), inspiration = body.inspirationLink === null || body.inspirationLink === void 0 ? null : typeof body.inspirationLink === "string" && body.inspirationLink.length <= 2e3 ? body.inspirationLink : void 0;
		if (inspiration === void 0) throw validation("inspirationLink", "Enter a valid inspiration link.");
		const now = Date.now();
		if (!(await db.prepare(`UPDATE style_profiles SET answers=?,current_section=?,inspiration_link=?,updated_at=? WHERE id=? AND status IN ('draft','reopened')`).bind(JSON.stringify(answers), body.currentSection, inspiration, now, access.profileId).run()).meta.changes) throw validation("profile", "This submitted profile is locked.");
		return dataResponse({
			status: "saved",
...
//#endregion
//#region app/client-actions/[token]/page.tsx
var page_exports$1 = /* @__PURE__ */ __exportAll({
	default: () => ClientActionPage,
	dynamic: () => dynamic$1
});
var dynamic$1 = "force-dynamic";
async function ClientActionPage({ params }) {
	const { token } = await params;
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(ClientActionClient, { token });
}
//#endregion
//#region app/style-profile/[token]/page.tsx
var page_exports = /* @__PURE__ */ __exportAll({
	default: () => PrivateStyleProfilePage,
	dynamic: () => dynamic
});
var dynamic = "force-dynamic";
async function PrivateStyleProfilePage({ params }) {
	const { token } = await params;
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(StyleProfileClient, { token });
}
//#endregion
//#region lib/server/booking-policy.ts
var allowed = {
	confirm: new Set(["pending", "change_proposed"]),
	decline: new Set(["pending", "change_proposed"]),
	"release-hold": new Set(["pending", "change_proposed"]),
	"propose-time": new Set(["pending", "change_proposed"]),
	cancel: new Set(["confirmed"]),
	complete: new Set(["confirmed"])
};
function assertAdminActionState(action, status) {
	if (!allowed[action].has(status)) throw new ApiError(409, "INVALID_BOOKING_STATE", `The ${action.replace("-", " ")} action is not available for a ${status.replace("_", " ")} booking.`);
}
function addCalendarYears(epoch, years) {
	const date = new Date(epoch), month = date.getUTCMonth();
	date.setUTCFullYear(date.getUTCFullYear() + years);
	if (date.getUTCMonth() !== month) date.setUTCDate(0);
	return date.getTime();
}
//#endregion
//#region app/api/admin/bookings/[bookingId]/[action]/route.ts
var route_exports = /* @__PURE__ */ __exportAll({ POST: () => POST });
async function POST(request, ctx) {
	return withApi(async (id) => {
		const admin = requireAdmin(request, true), { bookingId, action } = await ctx.params;
		if (!new Set([
			"confirm",
			"decline",
			"release-hold",
			...
			"cancel",
			"complete"
		]).has(action)) throw new ApiError(404, "ACTION_NOT_FOUND", "Booking action not found.");
		const body = await readJsonObject(request);
		rejectUnexpectedKeys(body, [
			"expectedStatus",
			"reason",
		...
		const expected = requiredString(body.expectedStatus, "expectedStatus", 40), db = getD1(), b = await load(db, bookingId);
		if (!b) throw new ApiError(404, "BOOKING_NOT_FOUND", "Booking request not found.");
		if (b.status !== expected) throw new ApiError(409, "BOOKING_STATE_CHANGED", "This request changed. Refresh before continuing.");
		assertAdminActionState(action, b.status);
		...
		let profileAccessUrl = null, profileAccessExpiresAt = null, clientActionUrl = null, clientActionExpiresAt = null;
		if (action === "confirm") {
			...
			const rawToken = randomPrivateToken(), tokenHash = await hashPrivateToken(rawToken);
			profileAccessExpiresAt = now + STYLE_PROFILE_TOKEN_TTL_MS;
			profileAccessUrl = `${new URL(request.url).origin}/style-profile/${rawToken}`;
			await db.batch([
				...
				history(db, bookingId, expected, "confirmed", admin.subjectId, reason, {
					action,
					idempotencyKeyHash: keyHash,
					profileLinkExpiresAt: new Date(profileAccessExpiresAt).toISOString()
				}, now),
				communication(db, bookingId, b.clientEmail, "booking_confirmed", now)
			]);
		} else if (action === "decline") {
			const why = requiredString(body.reason, "reason", 500);
			await db.batch([
				db.prepare(`UPDATE bookings SET status='declined',declined_at=?,updated_at=? WHERE id=? AND status=?`).bind(now, now, bookingId, expected),
				db.prepare(`UPDATE booking_holds SET active=0,released_at=?,release_reason=? WHERE booking_id=? AND active=1 AND EXISTS(SELECT 1 FROM bookings WHERE id=? AND status='declined')`).bind(now, why, bookingId, bookingId),
				history(db, bookingId, expected, "declined", admin.subjectId, why, {
					action,
					idempotencyKeyHash: keyHash
				}, now),
				communication(db, bookingId, b.clientEmail, "booking_declined", now)
			]);
		} else if (action === "release-hold") {
			if (!b.holdId) throw new ApiError(409, "NO_ACTIVE_HOLD", "This request no longer has an active hold.");
			const why = reason ?? "Released by Kayla";
			await db.batch([db.prepare(`UPDATE booking_holds SET active=0,released_at=?,release_reason=? WHERE id=? AND active=1`).bind(now, why, b.holdId), history(db, bookingId, expected, expected, admin.subjectId, why, {
				action,
				idempotencyKeyHash: keyHash
			}, now)]);
			...
			const text = requiredString(body.requestedStartAt, "requestedStartAt", 40), start = Date.parse(text);
			if (!Number.isFinite(start) || new Date(start).toISOString() !== text) throw validation("requestedStartAt", "Choose a valid available time.");
			const date = localDateKey(start, "America/Boise"), slot = (await calculateAvailability(db, b.serviceCode, date, date, Date.now(), bookingId)).slots.find((x) => x.startsAt === text);
			if (!slot) throw new ApiError(409, "SLOT_UNAVAILABLE", "That alternate time is unavailable.");
			const end = Date.parse(slot.endsAt), holdId = crypto.randomUUID(), rawActionToken = randomPrivateToken(), actionTokenHash = await hashPrivateToken(rawActionToken);
			clientActionExpiresAt = Math.min(start, now + STYLE_PROFILE_TOKEN_TTL_MS);
			clientActionUrl = `${new URL(request.url).origin}/client-actions/${rawActionToken}`;
			await db.batch([
				...
				db.prepare(`UPDATE private_access_tokens SET revoked_at=? WHERE booking_id=? AND purpose='alternate_time' AND revoked_at IS NULL`).bind(now, bookingId),
				...
				history(db, bookingId, expected, "change_proposed", admin.subjectId, reason, {
					action,
					idempotencyKeyHash: keyHash,
					clientActionExpiresAt: new Date(clientActionExpiresAt).toISOString()
				}, now),
				communication(db, bookingId, b.clientEmail, "alternate_time_proposed", now)
			]);
		} else if (action === "cancel") {
			const why = requiredString(body.reason, "reason", 500);
			await db.batch([
				db.prepare(`UPDATE bookings SET status='cancelled',cancelled_at=?,updated_at=? WHERE id=? AND status='confirmed'`).bind(now, now, bookingId),
				db.prepare(`UPDATE booking_holds SET active=0,released_at=?,release_reason=? WHERE booking_id=? AND active=1`).bind(now, why, bookingId),
				db.prepare(`UPDATE private_access_tokens SET revoked_at=? WHERE booking_id=? AND revoked_at IS NULL`).bind(now, bookingId),
				history(db, bookingId, "confirmed", "cancelled", admin.subjectId, why, {
					action,
					idempotencyKeyHash: keyHash
				}, now),
				communication(db, bookingId, b.clientEmail, "booking_cancelled", now)
			]);
		} else {
			const retention = addCalendarYears(now, 2);
			await db.batch([
				db.prepare(`UPDATE bookings SET status='completed',completed_at=?,updated_at=? WHERE id=? AND status='confirmed'`).bind(now, now, bookingId),
				db.prepare(`UPDATE booking_holds SET active=0,released_at=?,release_reason='Appointment completed' WHERE booking_id=? AND active=1`).bind(now, bookingId),
				db.prepare(`UPDATE clients SET last_completed_appointment_at=?,retention_delete_after=?,updated_at=? WHERE id=?`).bind(now, retention, now, b.clientId),
				db.prepare(`UPDATE style_profiles SET retention_delete_after=?,updated_at=? WHERE booking_id=?`).bind(retention, now, bookingId),
				history(db, bookingId, "confirmed", "completed", admin.subjectId, reason, {
					action,
					idempotencyKeyHash: keyHash,
					retentionDeleteAfter: new Date(retention).toISOString()
				}, now)
			]);
		}
		const updated = await load(db, bookingId);
		if (!updated || action !== "release-hold" && updated.status === expected) throw new ApiError(409, "BOOKING_STATE_CHANGED", "This request changed. Refresh before continuing.");
		return dataResponse({
			bookingId,
			status: updated.status,
			holdActive: Boolean(updated.holdId),
			profileAccessUrl,
			profileAccessExpiresAt: profileAccessExpiresAt ? new Date(profileAccessExpiresAt).toISOString() : null,
			clientActionUrl,
			clientActionExpiresAt: clientActionExpiresAt ? new Date(clientActionExpiresAt).toISOString() : null
		}, 200, id);
	});
}
async function load(db, id) {
	return db.prepare(`SELECT b.id,b.status,b.profile_type AS profileType,b.client_id AS clientId,s.code AS serviceCode,c.email AS clientEmail,h.id AS holdId,h.starts_at AS holdStartsAt,h.ends_at AS holdEndsAt FROM bookings b JOIN services s ON s.id=b.service_id JOIN clients c ON c.id=b.client_id LEFT JOIN booking_holds h ON h.booking_id=b.id AND h.active=1 WHERE b.id=?`).bind(id).first();
}
function history(db, id, from, to, actor, reason, metadata, now) {
	return db.prepare(`INSERT INTO booking_status_history(id,booking_id,from_status,to_status,actor_type,actor_id,reason,metadata,created_at) SELECT ?,id,?,?,'admin',?,?,?,? FROM bookings WHERE id=? AND status=?`).bind(crypto.randomUUID(), from, to, actor, reason, JSON.stringify(metadata), now, id, to);
}
function communication(db, id, email, key, now) {
	return db.prepare(`INSERT INTO communications(id,booking_id,channel,template_key,recipient,status,metadata,created_at) SELECT ?,id,'email',?,?,'recorded',?,? FROM bookings WHERE id=?`).bind(crypto.randomUUID(), key, email, JSON.stringify({ deliveryDeferred: true }), now, id);
}
//#endregion
...
var routes = [
	{
		...
		ids: {
			"route": "route:/api/admin/bookings/:bookingId/export",
			"page": null,
			"routeHandler": "route-handler:/api/admin/bookings/:bookingId/export",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/bookings/:bookingId/export",
		patternParts: [
			"api",
			"admin",
			"bookings",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/bookings/:bookingId/profile",
			"page": null,
			"routeHandler": "route-handler:/api/admin/bookings/:bookingId/profile",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/bookings/:bookingId/profile",
		patternParts: [
			"api",
			"admin",
			"bookings",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/bookings/:bookingId/reopen-profile",
			"page": null,
			"routeHandler": "route-handler:/api/admin/bookings/:bookingId/reopen-profile",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/bookings/:bookingId/reopen-profile",
		patternParts: [
			"api",
			"admin",
			"bookings",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/bookings/:bookingId/select-profile-type",
			"page": null,
			"routeHandler": "route-handler:/api/admin/bookings/:bookingId/select-profile-type",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/bookings/:bookingId/select-profile-type",
		patternParts: [
			"api",
			"admin",
			"bookings",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/data-requests/:requestId/export",
			"page": null,
			"routeHandler": "route-handler:/api/admin/data-requests/:requestId/export",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/data-requests/:requestId/export",
		patternParts: [
			"api",
			"admin",
			"data-requests",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/client-actions/:token/accept-proposed-time",
			"page": null,
			"routeHandler": "route-handler:/api/client-actions/:token/accept-proposed-time",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/client-actions/:token/accept-proposed-time",
		patternParts: [
			"api",
			"client-actions",
			":token",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/client-actions/:token/request-another-time",
			"page": null,
			"routeHandler": "route-handler:/api/client-actions/:token/request-another-time",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/client-actions/:token/request-another-time",
		patternParts: [
			"api",
			"client-actions",
			":token",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/style-profile/:token/calendar",
			"page": null,
			"routeHandler": "route-handler:/api/style-profile/:token/calendar",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/style-profile/:token/calendar",
		patternParts: [
			"api",
			"style-profile",
			":token",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/style-profile/:token/submit",
			"page": null,
			"routeHandler": "route-handler:/api/style-profile/:token/submit",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/style-profile/:token/submit",
		patternParts: [
			"api",
			"style-profile",
			":token",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/settings/availability/overrides/:overrideId",
			"page": null,
			"routeHandler": "route-handler:/api/admin/settings/availability/overrides/:overrideId",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/settings/availability/overrides/:overrideId",
		patternParts: [
			"api",
			"admin",
			"settings",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/bookings/:bookingId",
			"page": null,
			"routeHandler": "route-handler:/api/admin/bookings/:bookingId",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/bookings/:bookingId",
		patternParts: [
			"api",
			"admin",
			"bookings",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/data-requests/:requestId",
			"page": null,
			"routeHandler": "route-handler:/api/admin/data-requests/:requestId",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/data-requests/:requestId",
		patternParts: [
			"api",
			"admin",
			"data-requests",
	...
	},
	{
		...
		ids: {
			"route": "route:/admin/availability",
			"page": "page:/admin/availability",
			"routeHandler": null,
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
		...
		patternParts: ["admin", "availability"],
		isDynamic: false,
		params: [],
		rootParamNames: [],
		page: page_exports$5,
		routeHandler: null,
		layouts: [layout_exports],
		routeSegments: ["admin", "availability"],
		templateTreePositions: [],
		layoutTreePositions: [0],
		templates: [],
		errors: [null],
	...
	},
	{
		...
		ids: {
			"route": "route:/admin/privacy",
			"page": "page:/admin/privacy",
			"routeHandler": null,
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
		...
		pattern: "/admin/privacy",
		patternParts: ["admin", "privacy"],
		isDynamic: false,
		params: [],
		rootParamNames: [],
		page: page_exports$4,
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/bookings",
			"page": null,
			"routeHandler": "route-handler:/api/admin/bookings",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/bookings",
		patternParts: [
			"api",
			"admin",
			"bookings"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/data-requests",
			"page": null,
			"routeHandler": "route-handler:/api/admin/data-requests",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/data-requests",
		patternParts: [
			"api",
			"admin",
			"data-requests"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/operations/maintenance",
			"page": null,
			"routeHandler": "route-handler:/api/admin/operations/maintenance",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/operations/maintenance",
		patternParts: [
			"api",
			"admin",
			"operations",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/settings/availability",
			"page": null,
			"routeHandler": "route-handler:/api/admin/settings/availability",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/settings/availability",
		patternParts: [
			"api",
			"admin",
			"settings",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/settings/availability/overrides",
			"page": null,
			"routeHandler": "route-handler:/api/admin/settings/availability/overrides",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/settings/availability/overrides",
		patternParts: [
			"api",
			"admin",
			"settings",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/availability",
			"page": null,
			"routeHandler": "route-handler:/api/availability",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		...
		layouts: [layout_exports],
		routeSegments: ["api", "availability"],
		templateTreePositions: [],
		layoutTreePositions: [0],
		templates: [],
		errors: [null],
	...
	},
	{
		...
		ids: {
			"route": "route:/api/bookings",
			"page": null,
			"routeHandler": "route-handler:/api/bookings",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/internal/maintenance",
			"page": null,
			"routeHandler": "route-handler:/api/internal/maintenance",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/internal/maintenance",
		patternParts: [
			"api",
			"internal",
			"maintenance"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/services",
			"page": null,
			"routeHandler": "route-handler:/api/services",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/client-actions",
			"page": "page:/client-actions",
			"routeHandler": null,
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
	...
	},
	{
		...
		ids: {
			"route": "route:/style-profile",
			"page": "page:/style-profile",
			"routeHandler": null,
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
		...
		pattern: "/style-profile",
		patternParts: ["style-profile"],
		isDynamic: false,
		params: [],
		rootParamNames: [],
		page: page_exports$2,
	...
	},
	{
		...
		ids: {
			"route": "route:/api/client-actions/:token",
			"page": null,
			"routeHandler": "route-handler:/api/client-actions/:token",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/client-actions/:token",
		patternParts: [
			"api",
			"client-actions",
			":token"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/style-profile/:token",
			"page": null,
			"routeHandler": "route-handler:/api/style-profile/:token",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/style-profile/:token",
		patternParts: [
			"api",
			"style-profile",
			":token"
	...
	},
	{
		...
		ids: {
			"route": "route:/client-actions/:token",
			"page": "page:/client-actions/:token",
			"routeHandler": null,
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
		...
		pattern: "/client-actions/:token",
		patternParts: ["client-actions", ":token"],
		isDynamic: true,
		params: ["token"],
		rootParamNames: [],
		page: page_exports$1,
		routeHandler: null,
		layouts: [layout_exports],
		routeSegments: ["client-actions", "[token]"],
		templateTreePositions: [],
		layoutTreePositions: [0],
		templates: [],
		errors: [null],
	...
	},
	{
		...
		ids: {
			"route": "route:/style-profile/:token",
			"page": "page:/style-profile/:token",
			"routeHandler": null,
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
		...
		pattern: "/style-profile/:token",
		patternParts: ["style-profile", ":token"],
		isDynamic: true,
		params: ["token"],
		rootParamNames: [],
		page: page_exports,
		routeHandler: null,
		layouts: [layout_exports],
		routeSegments: ["style-profile", "[token]"],
		templateTreePositions: [],
		layoutTreePositions: [0],
		templates: [],
		errors: [null],
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/bookings/:bookingId/:action",
			"page": null,
			"routeHandler": "route-handler:/api/admin/bookings/:bookingId/:action",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/bookings/:bookingId/:action",
		patternParts: [
			"api",
			"admin",
			"bookings",
...
var __publicFiles = new Set([
	...
	"/images/stylewithkayla_logo.png",
	"/window.svg"
]);
...
var _virtual_vinext_rsc_entry_default = createAppRscHandler({
	...
	setNavigationContext: setAppNavigationContext,
	staticParamsMap: {
		"/client-actions/:token": null,
		"/style-profile/:token": null
	},
